...
public class BlockPropertyTypeFileCutter {
    ...
    public static void main(String[] args) {
        var reader = new InputStreamReader(new BufferedInputStream(Objects.requireNonNull(
                BlockAttributesTest.class.getClassLoader().getResourceAsStream(BLOCK_PROPERTY_TYPES_FILE_PATH))));
        var parser = JsonParser.parseReader(reader);
        var output = new HashMap<String, String>();
        for (var entry :
                parser.getAsJsonObject().getAsJsonObject("propertyTypes").entrySet()) {
