...
public class BlockStateRegistry {
    ...
    private final ExecutorService asyncStateRemover =
            Executors.newSingleThreadExecutor(t -> new Thread(t, "BlockStateRegistry#asyncStateRemover"));
        ...
        try (InputStream stream = Server.class.getModule().getResourceAsStream("canonical_block_states.nbt")) {
            ...
            try (BufferedInputStream bis = new BufferedInputStream(stream)) {
                ...
                while (bis.available() > 0) {
                    ...
                    tag.putInt(
                            "blockId",
                            persistenceNameToBlockId.getOrDefault(
                                    tag.getString("name").toLowerCase(Locale.ENGLISH), -1));
                    tags.add(tag);
                }
            }
        } catch (IOException e) {
    ...
    private void registerStateId(CompoundTag block, int runtimeId) {
        ...
        CompoundTag pureTag = block.clone().remove("blockId").remove("version").remove("runtimeId");
        if (!pureTag.contains("states")) pureTag.putCompound("states", new CompoundTag());
        else
            pureTag.putCompound(
                    "states",
                    new CompoundTag(new TreeMap<>(pureTag.getCompound("states").getTags())));
        Registration registration = new Registration(
                null,
                runtimeId,
                MinecraftNamespaceComparator.fnv1a_32(NBTIO.write(pureTag, ByteOrder.LITTLE_ENDIAN)),
                block);

        Registration old = stateIdRegistration.putIfAbsent(stateId, registration);
        if (old != null && !old.equals(registration)) {
            throw new UnsupportedOperationException(
                    "The persistence NBT registration tried to replaced a runtime id. Old:" + old + ", New:" + runtimeId
                            + ", State:" + stateId);
        }
    ...
    private void registerState(int blockId, int meta, CompoundTag originalState, int runtimeId) {
        ...
        Registration registration =
                new Registration(state, runtimeId, state.getBlock().computeBlockStateHash(), null);
    ...
    @NotNull private String getStateId(CompoundTag block) {
    ...
    @Nullable private static Registration findRegistrationByRuntimeId(int runtimeId) {
        return runtimeIdRegistration.get(runtimeId);
    }

    @Nullable private BlockState buildStateFromCompound(CompoundTag block) {
    ...
    private Registration findRegistrationByStateId(BlockState state) {
        ...
        try {
            ...
            try {
                log.fatal(
                        "An error has occurred while trying to get the stateId of state: "
                                + "{}:{}"
                                + " - {}"
                                + " - {}",
                        state.getBlockId(),
        ...
        try {
            ...
            log.fatal(
                    "An error has occurred while trying to parse the legacyStateId of {}:{}",
                    state.getBlockId(),
                    state.getDataStorage(),
                    e);
        }
    ...
    private void removeStateIdsAsync(@Nullable Registration registration) {
        if (registration != null && registration != updateBlockRegistration) {
            asyncStateRemover.submit(
                    () -> stateIdRegistration.values().removeIf(r -> r.runtimeId == registration.runtimeId));
        }
    }

    private Registration logDiscoveryError(BlockState state) {
        log.error(
                "Found an unknown BlockId:Meta combination: {}:{}"
                        + " - {}"
                        + " - {}"
                        + " - {}"
    ...
    public int getRuntimeId(BlockState state) {
        String blockMapping = RuntimeItemMapping.getBlockMapping()
                .getOrDefault(state.getBlockId() + ":" + state.getDataStorage().intValue(), null);
    ...
    @DeprecationDetails(
            ...
            replaceWith = "getRuntimeId(BlockState state)",
            since = "1.3.0.0-PN")
    ...
    @PowerNukkitOnly
    @Since("1.5.2.0-PN")
    @Nullable public String getKnownBlockStateIdByRuntimeId(int runtimeId) {
    ...
    @PowerNukkitOnly
    @Since("1.4.0.0-PN")
    @Nullable public BlockState getBlockStateByRuntimeId(int runtimeId) {
        Registration registration = findRegistrationByRuntimeId(runtimeId);
        if (registration == null) {
            return null;
        }
    ...
    @PowerNukkitOnly
    @Since("1.4.0.0-PN")
    @NotNull public String getPersistenceName(int blockId) {
        String persistenceName = blockIdToPersistenceName.get(blockId);
        if (persistenceName == null) {
            String fallback = "blockid:" + blockId;
            log.warn(
                    "The persistence name of the block id {} is unknown! Using {} as an alternative!",
                    blockId,
                    fallback);
    ...
    public void registerPersistenceName(int blockId, String persistenceName) {
        synchronized (blockIdToPersistenceName) {
            ...
            if (oldName != null && !persistenceName.equalsIgnoreCase(oldName)) {
                throw new UnsupportedOperationException(
                        "The persistence name registration tried to replaced a name. Name:" + persistenceName + ", Old:"
                                + oldName + ", Id:" + blockId);
            }
            ...
            if (oldId != null && blockId != oldId) {
                ...
                throw new UnsupportedOperationException(
                        "The persistence name registration tried to replaced an id. Name:" + persistenceName
                                + ", OldId:" + oldId + ", Id:" + blockId);
            }
        }
    }
    ...
    @PowerNukkitOnly
    @Since("1.6.0.0-PNX")
    public static synchronized OK<?> registerCustomBlockState(List<CustomBlock> blockCustoms) {
        // 清空原本的数据
            ...
            try (BufferedInputStream bis = new BufferedInputStream(stream)) {
                while (bis.available() > 0) {
                    ...
                    tag.putInt(
                            "blockId",
                            persistenceNameToBlockId.getOrDefault(
                                    tag.getString("name").toLowerCase(), -1));
        ...
        for (var blockCustom : blockCustoms) {
            ...
            if (blockCustom instanceof Block block) {
                var properties = block.getProperties().getAllProperties().stream()
                        .map(BlockProperties.RegisteredBlockProperty::getProperty)
                        .toList();
                ...
                for (var eachProperty : properties) {
                    ...
                    if (stateNbtList == null) {
                        if (eachProperty instanceof BooleanBlockProperty) {
                            newStateNbtList.add(
                                    new LinkedCompoundTag("states").putBoolean(eachProperty.getName(), false));
                            newStateNbtList.add(
                                    new LinkedCompoundTag("states").putBoolean(eachProperty.getName(), true));
                            ...
                            for (long i = unsignedIntBlockProperty.getMinValue();
                                    ...
                                    i++) {
                                newStateNbtList.add(
                                        new LinkedCompoundTag("states").putInt(eachProperty.getName(), (int) i));
                            }
                            ...
                            if (arrayBlockProperty.isOrdinal()) {
                                ...
                                for (int i = 0, universeLength = universe.length; i < universeLength; i++) {
                                    newStateNbtList.add(
                                            new LinkedCompoundTag("states").putInt(eachProperty.getName(), i));
                                }
                            } else {
                                for (var each : arrayBlockProperty.getUniverse()) {
                                    newStateNbtList.add(new LinkedCompoundTag("states")
                                            .putString(eachProperty.getName(), each.toString()));
                                }
                            }
                        }
                    } else {
                        for (var stateNbt : stateNbtList) {
                            if (eachProperty instanceof BooleanBlockProperty) {
                                ...
                                if (arrayBlockProperty.isOrdinal()) {
                                    ...
                                    for (var each : arrayBlockProperty.getUniverse()) {
                                        newStateNbtList.add(
                                                stateNbt.copy().putString(eachProperty.getName(), each.toString()));
                                    }
                                }
                            }
                        }
    ...
    @PowerNukkitOnly
    @Since("1.4.0.0-PN")
    @NotNull public byte[] getBlockPaletteBytes() {
    ...
    @NotNull public BlockProperties getProperties(int blockId) {
    ...
    @PowerNukkitOnly
    @Since("1.4.0.0-PN")
    @NotNull public MutableBlockState createMutableState(int blockId) {
        return getProperties(blockId).createMutableState(blockId);
    }

    @PowerNukkitOnly
    @Since("1.4.0.0-PN")
    @NotNull public MutableBlockState createMutableState(int blockId, int bigMeta) {
    ...
    @PowerNukkitOnly
    @Since("1.4.0.0-PN")
    @NotNull public MutableBlockState createMutableState(int blockId, Number storage) {
        MutableBlockState blockState = createMutableState(blockId);
    ...
    @PowerNukkitOnly
    @Since("1.4.0.0-PN")
    @Nullable public Integer getBlockId(String persistenceName) {
    ...
    private static class Registration {
        ...
        @Nullable private CompoundTag originalBlock;
    }
