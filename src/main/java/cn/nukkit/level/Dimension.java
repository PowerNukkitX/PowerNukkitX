...
@Slf4j
public class Dimension implements Metadatable {
    // region finals - number finals
    public static final Dimension[] EMPTY_ARRAY = new Dimension[0];
    ...
    /**
     * Performs the hash set<> operation.
     */
    ...
    /**
     * Performs the long2 object non blocking map<> operation.
     */
    ...
    /**
     * Performs the int2 object open hash map<> operation.
     */
    private final Int2ObjectOpenHashMap<ChunkLoader> loaders = new Int2ObjectOpenHashMap<>();
    /**
     * Performs the int2 int open hash map operation.
     */
    ...
    /**
     * Performs the long array set operation.
     */
    @NonComputationAtomic
    ...
    /**
     * Performs the long open hash set operation.
     */
    ...
    /**
     * Performs the long2 object open hash map<> operation.
     */
    ...
    /**
     * Performs the concurrent linked deque<> operation.
     */
    ...
    /**
     * Performs the long2 int open hash map operation.
     */
    private final Long2IntMap chunkTickList = new Long2IntOpenHashMap();
    /**
     * Performs the simple vibration manager operation.
     */
    ...
    /**
     * When TPS is low, the tick-rate optimization delay increases.
     * CPU-intensive tasks should run only once per this many ticks.
     */
    ...
    /**
     * Performs the thread local random.current operation.
     */
    ...
    /**
     * Performs the long2 object maps.synchronize operation.
     */
    ...
    /// base tick system (owned by Level)
    @Getter
    ...
    /**
     * Performs the object2 int open hash map<string> operation.
     */
     ...
     */
    public Dimension(Level level, String name, String path, int dimSum, Class<? extends LevelProvider> provider, LevelConfig.GeneratorConfig generatorConfig) {
        ...
        try {
            ...
            this.generator.setLevel(Dimension.this);
        } catch (Throwable e) {
            throw new RuntimeException(e);
        }

        try {
            this.provider = new AtomicReference<>(provider.getConstructor(Dimension.class, String.class).newInstance(this, path));
        ...
        this.baseTickGameLoop = level.getBaseTickGameLoop();
        this.subTickGameLoop = level.getSubTickGameLoop();
    }

    /**
     * Returns whether random tick.
     */
    ...
    /**
     * Performs the chunk hash operation.
     */
    ...
    /**
     * Performs the block hash operation.
     */
    ...
    /**
     * Performs the unhash block operation.
     */
    ...
    /**
     * Performs the local block hash operation.
     */
    ...
    /**
     * Performs the chunk block hash operation.
     */
    public static int chunkBlockHash(int x, int y, int z) {
        return (x << 13) | (z << 9) | (y + 64); // For 384-height worlds, y needs one extra bit.
    }
    ...
    /**
     * Performs the generate chunk loader id operation.
     */
    ...
    /**
     * Performs the recalc tick opt delay operation.
     */
    ...
    /**
     * Returns whether high light chunk.
     */
    public boolean isHighLightChunk(int chunkX, int chunkZ) {
        return highLightChunks.contains(Dimension.chunkHash(chunkX, chunkZ));
    }
    ...
    public boolean unload(boolean force) {
        ...
        Dimension defaultDimension = defaultLevel != null ? defaultLevel.getOverworld() : null;
        if (this == defaultDimension && !force) {
            ev.setCancelled();
        }
        ...
        for (Player player : this.getPlayers().values().toArray(Player.EMPTY_ARRAY)) {
            if (defaultDimension == null || this == defaultDimension) {
                player.close(player.getLeaveMessage(), "Forced default level unload");
            } else {
                player.teleport(defaultDimension.getSafeSpawn());
            }
        }
    ...
    /**
     * Performs the register chunk loader operation.
     */
    ...
    /**
     * Performs the unregister chunk loader operation.
     */
    public boolean unregisterChunkLoader(ChunkLoader loader, int chunkX, int chunkZ, boolean isSafeUnload) {
        ...
        long chunkHash = Dimension.chunkHash(chunkX, chunkZ);
    ...
    /**
     * Performs the check time operation.
     */
    ...
    /**
     * Performs the send time operation.
     */
    ...
    /**
     * Performs the release tick cached blocks operation.
     */
    ...
    /**
     * Performs the do tick operation.
     */
    ...
    public void doTick(int currentTick) {
        ...
        try {
            ...
            // Check highlight chunks (3x3 around each player).
                    ...
                    if (player.isOnline()) {
                        ...
                        for (int dx = -1; dx <= 1; dx++) {
                            for (int dz = -1; dz <= 1; dz++) {
                                highLightChunks.add(Dimension.chunkHash(chunkX + dx, chunkZ + dz));
                            }
                        }
                    }
                }
    ...
    /**
     * Performs the check weather operation.
     */
    ...
    /**
     * Returns the dimension destination level.
     */
    public Dimension getDimensionDestinationLevel(int dimension) {
        if (dimensionCount > 1) {
    ...
    /**
     * Performs the adjust pos to nearby entity operation.
     */
    ...
    /**
     * Performs the check sleep operation.
     */
    public void checkSleep() {
        ...
        if (playerCount > 0 && sleepingPlayerCount / playerCount * 100 >= this.gameRules.getInteger(GameRule.PLAYERS_SLEEPING_PERCENTAGE)) {
            int time = this.getTime() % Dimension.TIME_FULL;

            if (isNight() || isThundering()) {
                this.setTime(this.getTime() + Dimension.TIME_FULL - time);
    ...
    /**
     * Performs the send block extra data operation.
     */
    ...
    /**
     * Performs the send blocks operation.
     */
    ...
    public void sendBlocks(Player[] target, Vector3[] blocks, int flags, int dataLayer, boolean optimizeRebuilds) {
        ...
        for (Vector3 b : blocks) {
            ...
            if (optimizeRebuilds) {
                long index = Dimension.chunkHash((int) b.x >> 4, (int) b.z >> 4);
            ...
            if (!this.loaders.isEmpty()) {
                for (ChunkLoader loader : this.loaders.values()) {
                    ...
                    for (int chunk = 0; chunk < chunksPerLoader; ++chunk) {
                        ...
                        long hash = Dimension.chunkHash(dx + chunkX, dz + chunkZ);
    ...
    /**
     * Updates the comparator output level.
     */
    ...
    /**
     * Updates the comparator output level selective.
     */
    ...
    /**
     * Performs the neighbor change around immediately operation.
     */
    ...
    /**
     * Immediately sends neighborChange updates for blocks around the specified position.
     *
    ...
    /**
     * Performs the schedule update operation.
     */
    ...
    /**
     * Returns whether cel scheduled update.
     */
    ...
    /**
     * Returns whether update scheduled.
     */
    ...
    /**
     * Returns whether concurrent schedule.
     */
    ...
    /**
     * Returns whether block tick pending.
     */
    ...
    /**
     * Returns the block update scheduler.
     */
    ...
    /**
     * Returns the pending block updates.
     */
    ...
    /**
     * Performs the scan blocks operation.
     */
    ...
    /**
     * Performs the raycast blocks operation.
     */
    ...
    /**
     * Performs the blocks block sight operation.
     */
    ...
    /**
     * Returns the tick cached collision blocks.
     */
    ...
    /**
     * Performs the fast collision cubes operation.
     */
    ...
    /**
     * Performs the calculate skylight subtracted operation.
     */
    ...
    /**
     * Performs the calculate celestial angle operation.
     */
    ...
    public Block getTickCachedBlock(int x, int y, int z, int layer, boolean load) {
        return tickCachedBlocks.computeIfAbsent(Dimension.chunkHash(x >> 4, z >> 4), key -> new SimpleTickCachedBlockStore(this))
    ...
    /**
     * Performs the compute remove block light operation.
     */
    ...
    /**
     * Performs the compute spread block light operation.
     */
    ...
    public void addBlockLightUpdate(int x, int y, int z) {
        ...
        synchronized (blockLightQueue) {
            ...
            blockSet.add(Dimension.localBlockHash(x, y, z, this));
        }
    }
    ...
    public boolean setBlock(int x, int y, int z, int layer, Block block, boolean direct, boolean update) {
        ...
        int cx = x >> 4;
        int cz = z >> 4;
        long index = Dimension.chunkHash(cx, cz);
    ...
    /**
     * Adds the block change.
     */
    private void addBlockChange(int x, int y, int z) {
        long index = Dimension.chunkHash(x >> 4, z >> 4);
        addBlockChange(index, x, y, z);
    }
        ...
        synchronized (changedBlocks) {
            ...
            if (currentMap != changeBlocksFullMap && currentMap != null) {
                if (currentMap.size() > MAX_BLOCK_CACHE) {
                    ...
                    currentMap.put(Dimension.localBlockHash(x, y, z, this), changeBlocksPresent);
                }
            }
        }
    }
    ...
    /**
     * Performs the drop and get item operation.
     */
    ...
    /**
     * Performs the use break on operation.
     */
    ...
    /**
     * Performs the drop exp orb operation.
     */
    ...
    /**
     * Performs the drop exp orb and get entities operation.
     */
    ...
    /**
     * Performs the use item on operation.
     */
    ...
    /**
     * Performs the before place block operation.
     */
    ...
    /**
     * Performs the place block operation.
     */
    @Nullable
    ...
    /**
     * Returns whether in spawn radius.
     */
    ...
    /**
     * Returns the colliding entities.
     */
    ...
    /**
     * Performs the fast colliding entities operation.
     */
    ...
    /**
     * Performs the stream colliding entities operation.
     */
    ...
    /**
     * Returns the nearby entities safe.
     */
    ...
    /**
     * Performs the fast nearby entities operation.
     */
    ...
    /**
     * Returns the entities from buffer.
     */
    ...
    /**
     * Returns the block entity by id.
     */
    ...
    /**
     * Returns the block entity if loaded.
     */
    ...
    /**
     * Returns the chunk block entities.
     */
    ...
    /**
     * Performs the pick biome operation.
     */
    ...
    /**
     * Returns the generator object holder.
     */
    ...
    public BlockColor getMapColorAt(int x, int z) {
        ...
        // Where Z-axis height varies, darken or lighten the color.
        ...
        if (up instanceof BlockFlowingWater || up1 instanceof BlockFlowingWater) {
            ...
            if (block.y < 62) {
                // Below sea level.
                // Sea level is y=62. Farther below moves the color closer to ocean color.
    ...
    /**
     * Performs the brighter operation.
     */
    ...
    /**
     * Returns the map colored block at.
     */
    ...
    /**
     * Performs the are neighboring chunks loaded operation.
     */
    ...
    /**
     * Returns whether chunk generated.
     */
    ...
    /**
     * Returns whether chunk populated.
     */
    ...
    /**
     * Returns the fuzzy spawn location.
     */
    ...
    /**
     * Performs the request chunk operation.
     */
    ...
    /**
     * Performs the send chunk operation.
     */
    ...
    /**
     * Performs the sub tick operation.
     */
    ...
    /**
     * Performs the process chunk request operation.
     */
    ...
    /**
     * Performs the schedule block entity update operation.
     */
    ...
    /**
     * Whether the chunk is in use. Spawn chunks, chunks in ticking areas,
     * and chunks with {@link ChunkLoader} are considered in use.
     *
     * @param x the chunk x
     * @param z the chunk z
     * @return the boolean
    ...
    public boolean isChunkInUse(int x, int z) {
        return isChunkInUse(Dimension.chunkHash(x, z));
    }
    ...
    /**
     * Performs the force load chunk operation.
     */
    ...
    /**
     * Performs the queue unload chunk operation.
     */
    ...
    /**
     * Returns whether cel unload chunk request.
     */
    public void cancelUnloadChunkRequest(int x, int z) {
        this.cancelUnloadChunkRequest(Dimension.chunkHash(x, z));
    }
    ...
    /**
     * Performs the standable operation.
     */
    ...
    public boolean isTicked() {
        ...
        return this.level.isLoaded();
    }

    /**
     * Returns whether thread running.
     */
    public boolean isThreadRunning() {
        return this.level.isThreadRunning();
    }

    /**
     * Gets the elapsed time for this map (always accumulates).
     */
    public int getTime() {
        return (int) time;
    }
    ...
    public int getDayTime() {
        return this.getTime() % Dimension.TIME_FULL;
    }
    ...
    /**
     * Sets the elapsed time for this map.
     */
    ...
    /**
     * Performs the start time operation.
     */
    ...
    /**
     * Performs the regenerate chunk operation.
     */
    ...
    /**
     * Performs the sync regenerate chunk operation.
     */
    ...
    /**
     * Returns whether chunk generating.
     */
    public boolean isChunkGenerating(int x, int z) {
        return this.chunkGenerationQueue.getOrDefault(Dimension.chunkHash(x, z), false);
    }
    ...
    /**
     * Performs the sync generate chunk operation.
     */
    ...
    /**
     * Removes the from generate list.
     */
    ...
    /**
     * Asynchronously runs server memory garbage collection.
     */
    ...
    /**
     * Performs the send weather operation.
     */
    ...
    /**
     * Returns whether block indirectly getting powered.
     */
    ...
    /**
     * Performs the create portal operation.
     */
    ...
    /**
     * Returns whether ray colliding with blocks.
     */
    ...
    /**
     * Performs the ensure y operation.
     */
