...
public class PopulatorOceanRuin extends PopulatorStructure {
        ...
        return nbt -> {
            if (nbt.getString("id").equals(BlockEntity.STRUCTURE_BLOCK) && "chest".equals(nbt.getString("metadata"))) {
                ...
                Server.getInstance()
                        .getScheduler()
                        .scheduleDelayedTask(
                                new BlockActorSpawnTask(chunk.getProvider().getLevel(), tag), 2);
            }
        };
    }
    ...
    public void populate(ChunkManager level, int chunkX, int chunkZ, NukkitRandom random, FullChunk chunk) {
        ...
        if ((biome == EnumBiome.OCEAN.id
                        || biome == EnumBiome.FROZEN_OCEAN.id
                        || biome == EnumBiome.DEEP_OCEAN.id
                        || biome >= 44 && biome <= 50)
                && chunkX
    ...
    protected void placeRuin(
            ReadableStructureTemplate template, FullChunk chunk, int seed, boolean isLarge, int index) {
        ...
        template.placeInChunk(
                ...
                new StructurePlaceSettings()
                        ...
                        .setBlockActorProcessor(
                                isLarge ? getBigRuinProcessor(chunk, random) : getSmallRuinProcessor(chunk, random)));
        ...
        if (index != -1) {
            ...
            mossy.placeInChunk(
                    ...
                    new StructurePlaceSettings()
                            ...
                            .setBlockActorProcessor(
                                    isLarge
                                            ? getBigRuinProcessor(chunk, random)
                                            : getSmallRuinProcessor(chunk, random)));
            cracked.placeInChunk(
                    chunk,
                    random,
                    vec,
                    new StructurePlaceSettings()
