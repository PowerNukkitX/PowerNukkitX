...
public class MineshaftPieces {
    ...
    @Nullable
    // \\ MineshaftPiece::createRandomShaftPiece(MineshaftData
    ...
    private static MineshaftPiece createRandomShaftPiece(
    ...
    @Nullable
    // \\ MineshaftPiece::generateAndAddPiece(StructurePiece
    // *,std::vector<std::unique_ptr<StructurePiece,std::default_delete<StructurePiece>>,std::allocator<std::unique_ptr<StructurePiece,std::default_delete<StructurePiece>>>> &,Random &,int,int,int,int,int)
    private static MineshaftPiece generateAndAddPiece(
            ...
            int genDepth) {
        if (genDepth <= 8
                && Math.abs(x - piece.getBoundingBox().x0) <= 80
                && Math.abs(z - piece.getBoundingBox().z0) <= 80) {
            MineshaftPiece result = createRandomShaftPiece(
                    pieces, random, x, y, z, orientation, genDepth + 1, ((MineshaftPiece) piece).type);
            if (result != null) {
                pieces.add(result);
    ...
    public static class MineshaftRoom extends MineshaftPiece {
        ...
        public MineshaftRoom(CompoundTag tag) {
            super(tag);
            tag.getList("Entrances", IntArrayTag.class)
                    .getAll()
                    .forEach(arrayTag -> this.childEntranceBoxes.add(new BoundingBox(arrayTag.data)));
        }
        ...
        @Override
        // \\ MineshaftRoom::addChildren(StructurePiece
        ...
        public void addChildren(StructurePiece piece, List<StructurePiece> pieces, NukkitRandom random) {
            ...
            for (int x = 0; x < this.boundingBox.getXSpan(); x += 4) {
                ...
                MineshaftPiece next = generateAndAddPiece(
                        piece,
                        pieces,
                        random,
                        this.boundingBox.x0 + x,
                        this.boundingBox.y0 + random.nextBoundedInt(yOffset) + 1,
    ...
    public static class MineshaftCorridor extends MineshaftPiece {
                    ...
                    default:
                        if (target <= 1) {
                            generateAndAddPiece(
                                    ...
                                    random,
                                    this.boundingBox.x0,
                                    this.boundingBox.y0 - 1 + random.nextBoundedInt(3),
                                    this.boundingBox.z0 - 1,
                                    orientation,
                                    genDepth);
                        } else if (target == 2) {
        ...
        protected boolean createChest(
                ChunkManager level, BoundingBox boundingBox, NukkitRandom random, int x, int y, int z) {
            BlockVector3 vec = new BlockVector3(this.getWorldX(x, z), this.getWorldY(y), this.getWorldZ(x, z));

            if (boundingBox.isInside(vec)
                    && level.getBlockIdAt(vec.x, vec.y, vec.z) == Block.AIR
                    && level.getBlockIdAt(vec.x, vec.y - 1, vec.z) != Block.AIR) {
        ...
        public boolean postProcess(
            ...
            this.generateMaybeBox(
                    level, boundingBox, random, 50, 0, 2, 0, 2, 2, z1, BlockState.AIR, BlockState.AIR, false, false);
            if (this.spiderCorridor) {
                this.generateMaybeBox(
                        level, boundingBox, random, 60, 0, 0, 0, 2, 1, z1, COBWEB, BlockState.AIR, false, true);
            }
                ...
                for (int z = 0; z <= z1; ++z) {
                    ...
                    if (!block.equals(BlockState.AIR) && Block.solid[id] && !Block.transparent[id]) {
                        this.maybeGenerateBlock(
                                ...
                                boundingBox,
                                random,
                                this.isInterior(level, 1, 0, z, boundingBox) ? 70 : 90,
                                1,
                                0,
                                z,
                                RAIL__NS);
    ...
    public static class MineshaftCrossing extends MineshaftPiece {
        ...
        public MineshaftCrossing(
                int genDepth, BoundingBox boundingBox, @Nullable BlockFace orientation, PopulatorMineshaft.Type type) {
            super(genDepth, type);
        ...
        @Nullable public static BoundingBox findCrossing(
                List<StructurePiece> pieces, NukkitRandom random, int x, int y, int z, BlockFace orientation) {
            BoundingBox boundingBox = new BoundingBox(x, y, z, x, y + 3 - 1, z);
            if (random.nextBoundedInt(4) == 0) {
                boundingBox.y1 += 4;
        ...
        public boolean postProcess(
            ...
            for (int x = this.boundingBox.x0; x <= this.boundingBox.x1; ++x) {
                for (int z = this.boundingBox.z0; z <= this.boundingBox.z1; ++z) {
                    if (this.getBlock(level, x, this.boundingBox.y0 - 1, z, boundingBox)
                                    .equals(BlockState.AIR)
                            && this.isInterior(level, x, this.boundingBox.y0 - 1, z, boundingBox)) {
    ...
    public static class MineshaftStairs extends MineshaftPiece {
        ...
        @Nullable public static BoundingBox findStairs(
                List<StructurePiece> pieces, NukkitRandom random, int x, int y, int z, BlockFace orientation) {
