...
public abstract class VectorMath {
    ...
    public static BlockFace calculateFace(Vector3 base, Vector3 side) {
        Vector3 vector = side.subtract(base);
        BlockFace.Axis axis =
                vector.x() != 0 ? BlockFace.Axis.X : vector.z() != 0 ? BlockFace.Axis.Z : BlockFace.Axis.Y;
        double direction = vector.getAxis(axis);
        return BlockFace.fromAxis(
                direction < 0 ? BlockFace.AxisDirection.NEGATIVE : BlockFace.AxisDirection.POSITIVE, axis);
    }
    ...
    public static List<Vector3> getPassByVector3(Vector3 from, Vector3 to) {
        ...
        if (from.x() != to.x()) {
            for (int xCut = NukkitMath.ceilDouble(Math.min(from.x(), to.x()));
                    xCut < NukkitMath.floorDouble(Math.max(from.x(), to.x())) + 1;
                    xCut++) {
                var ratio = (xCut - from.x()) / (to.x() - from.x());
                Vector3 currentXCut = new Vector3(
                        xCut, from.y() + (to.y() - from.y()) * ratio, from.z() + (to.z() - from.z()) * ratio);
                if (xCut != lastXCut.x()) {
        ...
        if (from.z() != to.z()) {
            for (var xCut : xCuts) {
                ...
                for (int zCut = NukkitMath.ceilDouble(Math.min(xCut.from.z(), xCut.to.z()));
                        zCut < NukkitMath.floorDouble(Math.max(xCut.from.z(), xCut.to.z())) + 1;
                        zCut++) {
                    var ratio = (zCut - xCut.from.z()) / (xCut.to.z() - xCut.from.z());
                    Vector3 currentZCut = new Vector3(
        ...
        if (from.y() != to.y()) {
            for (var zCut : zCuts) {
                ...
                for (int yCut = NukkitMath.ceilDouble(Math.min(zCut.from.y(), zCut.to.y()));
                        yCut < NukkitMath.floorDouble(Math.max(zCut.from.y(), zCut.to.y())) + 1;
                        yCut++) {
                    var ratio = (yCut - zCut.from.y()) / (zCut.to.y() - zCut.from.y());
                    Vector3 currentYCut = new Vector3(
