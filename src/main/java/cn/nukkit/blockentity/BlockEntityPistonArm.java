...
public class BlockEntityPistonArm extends BlockEntitySpawnable {
    ...
    public void loadNBT() {
        ...
        if (namedTag.contains("facing")) {
            ...
            if (block instanceof Faceable faceable) this.facing = faceable.getBlockFace();
        ...
        if (namedTag.contains("AttachedBlocks")) {
            ...
            if (blocks != null && blocks.size() > 0) {
                for (int i = 0; i < blocks.size(); i += 3) {
                    this.attachedBlocks.add(
                            new BlockVector3(blocks.get(i).data, blocks.get(i + 1).data, blocks.get(i + 2).data));
                }
            }
