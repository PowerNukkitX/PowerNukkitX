    ...
    public int execute(CommandSender sender, String commandLabel, Map.Entry<String, ParamList> result, CommandLogger log) {
            ...
            for (Player target : targets) {
                ...
                if (item == null) {
                    ...
                    if (count == 0) {
                        log.addError("commands.clear.failure.no.items", getViewableName(sender, target)).output();
                    } else {
                        log.addSuccess("commands.clear.success", getViewableName(sender, target), String.valueOf(count)).output();
                    }
                } else if (maxCount == 0) {
                    int count = 0;
                    ...
                    if (count == 0) {
                        ...
                        log.addSuccess("commands.clear.testing", getViewableName(sender, target), String.valueOf(count)).output();
                    }
                } else if (maxCount == -1) {
                    int count = 0;
                    ...
                    if (remaining == maxCount) {
                        ...
                        log.addSuccess("commands.clear.success", getViewableName(sender, target), String.valueOf(maxCount - remaining)).output();
                    }
                }
            }
            return targets.size();
