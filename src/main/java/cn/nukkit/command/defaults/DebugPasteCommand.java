...
public class DebugPasteCommand extends TestCommand implements CoreCommand {
    ...
    public DebugPasteCommand(String name) {
        ...
        this.commandParameters.put(
                "clear", new CommandParameter[] {CommandParameter.newEnum("clear", new String[] {"clear"})});
        ...
        try {
            ...
            if (Files.isDirectory(pastesFolder)) {
                try (Stream<Path> listing = Files.list(pastesFolder)) {
                    listing.filter(DebugPasteCommand::filterValidPastes).forEach(file -> {
                        try {
                            Files.delete(file);
    ...
    private static void uploadLast(CommandSender sender) {
        ...
        try {
            ...
            if (!last.isPresent()) {
                sender.sendMessage(
                        "No debug pastes was found. Try to run \"/debugpaste upload\" to create a new one and send right away.");
                return;
            }
            ...
            if (Files.isRegularFile(urlPath)) {
                Optional<String> url = Files.lines(urlPath)
                        .filter(line -> line.startsWith("URL=http") && line.length() > 8)
                        .findFirst();
                if (url.isPresent()) {
                    ...
                    if (!(sender instanceof ConsoleCommandSender)) {
                        sender.sendMessage(
                                "The url is also being logged in the console for convenience, so you can do CTRL+C, CTRL+V if you have console access.");
                        log.info(
                                "The last debug paste {} was already uploaded to: {}",
                                lastPath.getFileName(),
                                directUrl);
                    }
                    return;
                }
                ...
                Files.move(
                        urlPath,
                        urlPath.resolveSibling(
                                fileName.substring(0, fileName.length() - 4) + System.currentTimeMillis() + ".url"));
            }
    ...
    private static void upload(CommandSender sender, Path zipPath) {
        ...
        try {
            ...
            try (BufferedInputStream in = new BufferedInputStream(connection.getInputStream());
                    InputStreamReader rd = connection.getContentEncoding() != null
                            ? new InputStreamReader(in, connection.getContentEncoding())
                ...
                Utils.writeFile(
                        zipPath.resolveSibling(zipPath.getFileName() + ".url").toString(),
                        "[InternetShortcut]" + System.lineSeparator() + "URL=" + publicUrl + System.lineSeparator());
            }
        } catch (Exception e) {
            log.error("Failed to upload the debugpaste {}", zipPath, e);
            sender.sendMessage(
                    "Failed to upload the debugpaste, the file is still available in your server directory.");
        }
    }

    @NotNull private static String eval(String... command) {
        try {
            try (InputStream in = Runtime.getRuntime().exec(command).getInputStream()) {
                return IOUtils.readInputStreamToString(in, Charset.defaultCharset())
                        .trim();
            }
        } catch (Exception e) {
        ...
        server.getScheduler().scheduleAsyncTask(new AsyncTask() {
            ...
            public void onRun() {
                ...
                try {
                    ...
                    Path dir = Files.createDirectories(dataPath.resolve("debugpastes/debugpaste-" + now))
                            .toAbsolutePath();
                    ...
                    Optional<Path> secondMostLatest = StreamSupport.stream(
                                    Files.newDirectoryStream(dataPath.resolve("logs"))
                                            .spliterator(),
                                    false)
                            ...
                            .max(((a, b) -> {
                                ...
                                try {
                                    aTime = Files.readAttributes(a, BasicFileAttributes.class)
                                            .creationTime();
                                } catch (IOException ignored) {
                                }
                                try {
                                    bTime = Files.readAttributes(b, BasicFileAttributes.class)
                                            .creationTime();
                                } catch (IOException ignored) {
                                }
                                ...
                                return HumanStringComparator.getInstance()
                                        .compare(
                    ...
                    Utils.copyFile(
                            dataPath.resolve("logs/server.log").toFile(),
                            dir.resolve("server-latest.log").toFile());
                    if (secondMostLatest.isPresent()) {
                        Utils.copyFile(
                                secondMostLatest.get().toFile(),
                                dir.resolve("server-second-most-latest.log.gz").toFile());
                    }

                    Utils.copyFile(
                            dataPath.resolve("nukkit.yml").toFile(),
                            dir.resolve("nukkit.yml").toFile());
                    Utils.copyFile(
                            dataPath.resolve("server.properties").toFile(),
                            dir.resolve("server.properties").toFile());

                    StringBuilder b = new StringBuilder();
                    b.append("\n# Server Information\n");
                    b.append("server.name: ").append(server.getName()).append('\n');
                    b.append("version.api: ").append(server.getApiVersion()).append('\n');
                    b.append("version.nukkit: ")
                            .append(server.getNukkitVersion())
                            .append('\n');
                    b.append("version.git: ").append(server.getGitCommit()).append('\n');
                    b.append("version.minecraft: ").append(server.getVersion()).append('\n');
                    b.append("version.protocol: ")
                            .append(ProtocolInfo.CURRENT_PROTOCOL)
                            .append('\n');
                    b.append("plugins:");
                    ...
                    b.append("cpu.runtime: ")
                            .append(ManagementFactory.getRuntimeMXBean().getUptime())
                            .append('\n');
                    b.append("cpu.processors: ")
                ...
                if (args.length == 1 && "upload".equalsIgnoreCase(args[0])) {
                    ...
                    sender.sendMessage(
                            "Review the file and scrub the files as you wish, and run \"/debugpaste upload last\" to make it available online");
                }
            }
        });
        return true;
