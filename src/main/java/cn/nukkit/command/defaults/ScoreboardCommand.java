...
public class ScoreboardCommand extends VanillaCommand {
    public ScoreboardCommand(String name) {
        ...
        this.commandParameters.put("objectives-add", new CommandParameter[] {
            CommandParameter.newEnum(
                    "category", false, new CommandEnum("ScoreboardObjectivesCategory", List.of("objectives"), false)),
        ...
        this.commandParameters.put("objectives-remove", new CommandParameter[] {
            CommandParameter.newEnum(
                    "category", false, new CommandEnum("ScoreboardObjectivesCategory", List.of("objectives"), false)),
            CommandParameter.newEnum(
                    "action", false, new CommandEnum("ScoreboardRemoveAction", List.of("remove"), false)),
            OBJECTIVES.get(false),
        });
        this.commandParameters.put("objectives-setdisplay-list-sidebar", new CommandParameter[] {
            CommandParameter.newEnum(
                    "category", false, new CommandEnum("ScoreboardObjectivesCategory", List.of("objectives"), false)),
            CommandParameter.newEnum(
                    "action", false, new CommandEnum("ScoreboardSetDisplayAction", List.of("setdisplay"), false)),
            CommandParameter.newEnum(
                    "displaySlot",
                    false,
                    new CommandEnum("ScoreboardDisplaySlotSortable", List.of("list", "sidebar"), false),
                    CommandParamOption.SUPPRESS_ENUM_AUTOCOMPLETION),
            OBJECTIVES.get(true),
            CommandParameter.newEnum(
                    "sortOrder",
                    true,
                    new CommandEnum("ScoreboardSortOrder", List.of("ascending", "descending"), false),
                    CommandParamOption.SUPPRESS_ENUM_AUTOCOMPLETION),
        });
        this.commandParameters.put("objectives-setdisplay-belowname", new CommandParameter[] {
            ...
            CommandParameter.newEnum(
                    "displaySlot",
                    false,
                    new CommandEnum("ScoreboardDisplaySlotNonSortable", List.of("belowname"), false)),
            OBJECTIVES.get(true)
        });
        this.commandParameters.put("players-add-remove-set", new CommandParameter[] {
            CommandParameter.newEnum(
                    "category", false, new CommandEnum("ScoreboardPlayersCategory", List.of("players"), false)),
            CommandParameter.newEnum(
                    "action",
                    false,
                    new CommandEnum("ScoreboardPlayersNumAction", List.of("add", "remove", "set"), false)),
        ...
        this.commandParameters.put("players-operation", new CommandParameter[] {
            CommandParameter.newEnum(
                    "category", false, new CommandEnum("ScoreboardPlayersCategory", List.of("players"), false)),
            CommandParameter.newEnum(
                    "action", false, new CommandEnum("ScoreboardOperationAction", List.of("operation"), false)),
        ...
        this.commandParameters.put("players-random", new CommandParameter[] {
            CommandParameter.newEnum(
                    "category", false, new CommandEnum("ScoreboardPlayersCategory", List.of("players"), false)),
            CommandParameter.newEnum(
                    "action", false, new CommandEnum("ScoreboardRandomAction", List.of("random"), false)),
            CommandParameter.newType("player", false, CommandParamType.WILDCARD_TARGET), // allow *
            OBJECTIVES.get(false),
            CommandParameter.newType(
                    "min", false, CommandParamType.WILDCARD_INT, new WildcardIntNode(Integer.MIN_VALUE)),
            CommandParameter.newType(
                    "max", false, CommandParamType.WILDCARD_INT, new WildcardIntNode(Integer.MAX_VALUE))
        });
        this.commandParameters.put("players-reset", new CommandParameter[] {
            CommandParameter.newEnum(
                    "category", false, new CommandEnum("ScoreboardPlayersCategory", List.of("players"), false)),
            CommandParameter.newEnum(
                    "action", false, new CommandEnum("ScoreboardResetAction", List.of("reset"), false)),
            CommandParameter.newType("player", false, CommandParamType.WILDCARD_TARGET), // allow *
            OBJECTIVES.get(true),
        });
            ...
            switch (result.getKey()) {
                case "objectives-add" -> {
                    ...
                    if (manager.containScoreboard(objectiveName)) {
                        log.addError("commands.scoreboard.objectives.add.alreadyExists", objectiveName)
                                .output();
                        return 0;
                    }
                    ...
                    if (list.hasResult(4)) {
                        manager.addScoreboard(
                                new Scoreboard(objectiveName, list.getResult(4), criteriaName, SortOrder.ASCENDING));
                    } else {
                        manager.addScoreboard(
                                new Scoreboard(objectiveName, objectiveName, criteriaName, SortOrder.ASCENDING));
                    }
                    log.addSuccess("commands.scoreboard.objectives.add.success", objectiveName)
                            .output();
                    return 1;
                }
                case "objectives-list" -> {
                    log.addSuccess(
                            TextFormat.GREEN + "%commands.scoreboard.objectives.list.count",
                            String.valueOf(manager.getScoreboards().size()));
                    for (var scoreboard : manager.getScoreboards().values()) {
                        log.addSuccess(
                                "commands.scoreboard.objectives.list.entry",
                                scoreboard.getObjectiveName(),
                                scoreboard.getDisplayName(),
                                scoreboard.getCriteriaName());
                    }
                    log.output(true);
                    return 1;
                }
                case "objectives-remove" -> {
                    String objectiveName = list.getResult(2);
                    if (!manager.containScoreboard(objectiveName)) {
                        log.addError("commands.scoreboard.objectiveNotFound", objectiveName)
                                .output();
                        return 0;
                    }
                    if (manager.removeScoreboard(objectiveName)) {
                        log.addSuccess("commands.scoreboard.objectives.remove.success", objectiveName)
                                .output();
                    }
                    return 1;
                }
                case "objectives-setdisplay-list-sidebar" -> {
                    ...
                    if (!list.hasResult(3)) {
                        ...
                        SortOrder order = list.hasResult(4)
                                ? switch (orderName) {
                ...
                case "players-list" -> {
                    ...
                    if (list.hasResult(2)) {
                        ...
                        if (scorers.isEmpty()) {
                            log.addError("commands.scoreboard.players.list.empty")
                                    .output();
                            return 0;
                        }
                        for (IScorer scorer : scorers) {
                            ...
                            if (!find) {
                                log.addError("commands.scoreboard.players.list.player.empty", scorer.getName())
                                        .output();
                                return 0;
                            }
                            log.addSuccess(
                                    TextFormat.GREEN + "%commands.scoreboard.players.list.player.count",
                        ...
                        manager.getScoreboards().values().forEach(scoreboard -> scoreboard
                                .getLines()
                                .values()
                                .forEach(line -> scorerNames.add(
                                        TextFormat.WHITE + line.getScorer().getName())));
                        log.addSuccess(
                                TextFormat.GREEN + "%commands.scoreboard.players.list.count",
                                String.valueOf(scorerNames.size()));
                ...
                case "players-random" -> {
                    ...
                    for (IScorer scorer : scorers) {
                        int score = (int) (min
                                + random.nextLong(max - min
                                        + 1)); // avoid "java.lang.IllegalArgumentException: bound must be positive"
                        if (!scoreboard.getLines().containsKey(scorer)) {
                ...
                case "players-test" -> {
                    ...
                    for (IScorer scorer : scorers) {
                        ...
                        if (line == null) {
                            log.addError("commands.scoreboard.players.score.notFound", objectiveName, scorer.getName())
                                    .output();
                            return 0;
                        }
            ...
            case "add" -> {
                ...
                if (count == 1) {
                    ...
                    log.addSuccess(
                            ...
                            objectiveName,
                            scorer.getName(),
                            String.valueOf(scoreboard.getLines().get(scorer).getScore()));
                } else {
                    log.addSuccess(
    ...
    private int playersOperate(ParamList list, CommandSender sender, IScoreboardManager manager, CommandLogger log)
        ...
        if (!manager.containScoreboard(selectorObjectiveName)) {
            log.addError("commands.scoreboard.objectiveNotFound", selectorObjectiveName)
                    .output();
            return 0;
        }
            ...
            for (IScorer selectorScorer : selectorScorers) {
                ...
                if (!selectorScoreboard.getLines().containsKey(selectorScorer)) {
                    log.addError(
                                    ...
                                    selectorScorer.getName())
                            .output();
                    return 0;
                }
                int targetScore = targetScoreboard.getLines().get(targetScorer).getScore();
                int selectorScore =
                        selectorScoreboard.getLines().get(selectorScorer).getScore();
                int changedScore = -1;
                switch (operation) {
                    case "+=" -> {
    ...
    private Set<IScorer> parseScorers(
            CommandSender sender, String wildcardTargetStr, @Nullable IScoreboard wildcardScoreboard)
            throws SelectorSyntaxException {
        var manager = Server.getInstance().getScoreboardManager();
