...
public record CommandLogger(Command command,
    ...
    private TranslationContainer broadcastMessage(String key, String[] value, CommandSender target) {
        ...
        String resultStr = "[" + (target.isPlayer() ? target.asPlayer().getDisplayName() : target.getName()) + ": " + (!message.getText().equals(target.getServer().getLanguage().get(message.getText())) ? "%" : "") + message.getText() + "]";
