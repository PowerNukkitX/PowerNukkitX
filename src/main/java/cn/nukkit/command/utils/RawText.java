...
public class RawText {
    ...
    private static void preParse(CommandSender sender, Component cps) {
        if (cps.getType() != Component.ComponentType.RAWTEXT) return;
        ...
        for (Component component : components.toArray(new Component[0])) {
            if (component.getType() == Component.ComponentType.SCORE) {
                ...
                if (newComponent != null) components.set(components.indexOf(component), newComponent);
    ...
    private static Component preParseScore(Component component, CommandSender sender) {
        ...
        if (name_str.equals("*")) {
            ...
            List<IScorer> scorers = EntitySelectorAPI.getAPI().matchEntities(sender, name_str).stream()
        ...
        if (value == null) value = scoreboard.getLine(scorer).getScore();
