...
public class InventoryTransactionProcessor extends DataPacketProcessor<InventoryTransactionPacket> {
    ...
    public void handle(@NotNull PlayerHandle playerHandle, @NotNull InventoryTransactionPacket pk) {
        ...
        for (NetworkInventoryAction networkInventoryAction : pk.actions) {
            if (networkInventoryAction.getInventorySource().getType() == InventorySource.Type.WORLD_INTERACTION) {
                if (networkInventoryAction.getInventorySource().getFlag() == InventorySource.Flag.DROP_ITEM) {
                    PlayerDropItemEvent ev;
                    player.getServer().getPluginManager().callEvent(ev = new PlayerDropItemEvent(player, networkInventoryAction.newItem));
