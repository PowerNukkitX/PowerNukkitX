 ...
 */
@Slf4j
public class EntityPickRequestProcessor extends DataPacketProcessor<EntityPickRequestPacket> {

    @Override
    public void handle(@NotNull PlayerHandle playerHandle, @NotNull EntityPickRequestPacket pk) {
        final Player player = playerHandle.player;
        final Entity entity = player.level.getEntity(pk.getEntityId());
        if (entity == null) {
    ...
    private CompoundTag getCleanedNBT(Entity entity) {
        entity.saveNBT();
        CompoundTag tag = entity.namedTag.copy();
        tag.remove("Pos").remove("Motion").remove("OnGround").remove("Rotation").remove("uuid");
