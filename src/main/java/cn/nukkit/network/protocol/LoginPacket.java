...
public class LoginPacket extends DataPacket {
    public static final int $1 = ProtocolInfo.LOGIN_PACKET;
    ...
    private void decodeChainData(BinaryStream binaryStream) {
        ...
        for (String c : chains) {
            JsonObject $3 = decodeToken(c);
            if (chainMap == null) continue;
            if (chainMap.has("extraData")) {
                if (chainMap.has("iat")) {
                    this.issueUnixTime = chainMap.get("iat").getAsLong() * 1000;
                }
                JsonObject $4 = chainMap.get("extraData").getAsJsonObject();
    ...
    private void decodeSkinData(BinaryStream binaryStream) {
        JsonObject $5 = decodeToken(new String(binaryStream.get(binaryStream.getLInt())));
        ...
        if (skinToken.has("SkinId")) {
            ...
            var $6 = skinToken.get("SkinId").getAsString();
    ...
    private static SkinAnimation getAnimation(JsonObject element) {
        float $7 = element.get("Frames").getAsFloat();
        int $8 = element.get("Type").getAsInt();
        byte[] data = Base64.getDecoder().decode(element.get("Image").getAsString());
        int $9 = element.get("ImageWidth").getAsInt();
        int $10 = element.get("ImageHeight").getAsInt();
        int $11 = element.get("AnimationExpression").getAsInt();
    ...
    private static SerializedImage getImage(JsonObject token, String name) {
        if (token.has(name + "Data")) {
            ...
            if (token.has(name + "ImageHeight") && token.has(name + "ImageWidth")) {
                int $12 = token.get(name + "ImageWidth").getAsInt();
                int $13 = token.get(name + "ImageHeight").getAsInt();
    ...
    private static PersonaPiece getPersonaPiece(JsonObject object) {
        String $14 = object.get("PieceId").getAsString();
        String $15 = object.get("PieceType").getAsString();
        String $16 = object.get("PackId").getAsString();
        boolean $17 = object.get("IsDefault").getAsBoolean();
        String $18 = object.get("ProductId").getAsString();
        return new PersonaPiece(pieceId, pieceType, packId, isDefault, productId);
    }

    public static PersonaPieceT
    /**
     * @deprecated 
     */
    int getTint(JsonObject object) {
        String $19 = object.get("PieceType").getAsString();
