...
public class NetworkInventoryAction {
    ...
    public NetworkInventoryAction read(InventoryTransactionPacket packet) {
        ...
        switch (type) {
            case UNTRACKED_INTERACTION_UI -> inventorySource =
                    InventorySource.fromUntrackedInteractionUI(packet.getVarInt());
            case CONTAINER -> {
            ...
            case NON_IMPLEMENTED_TODO -> {
                ...
                switch (this.windowId) {
                    case SOURCE_TYPE_CRAFTING_RESULT, SOURCE_TYPE_CRAFTING_USE_INGREDIENT -> packet.isCraftingPart =
                            true;
                    case SOURCE_TYPE_ENCHANT_INPUT,
                            SOURCE_TYPE_ENCHANT_OUTPUT,
                            SOURCE_TYPE_ENCHANT_MATERIAL -> packet.isEnchantingPart = true;
                    case SOURCE_TYPE_ANVIL_INPUT,
                            SOURCE_TYPE_ANVIL_MATERIAL,
                            SOURCE_TYPE_ANVIL_RESULT -> packet.isRepairItemPart = true;
                    case SOURCE_TYPE_TRADING_INPUT_1,
                            SOURCE_TYPE_TRADING_INPUT_2,
                            SOURCE_TYPE_TRADING_USE_INPUTS,
                            SOURCE_TYPE_TRADING_OUTPUT -> packet.isTradeItemPart = true;
                }
    ...
    public void write(InventoryTransactionPacket packet) {
        ...
        switch (inventorySource.getType()) {
            case CONTAINER, UNTRACKED_INTERACTION_UI, NON_IMPLEMENTED_TODO -> packet.putVarInt(
                    inventorySource.getContainerId());
            case WORLD_INTERACTION -> packet.putUnsignedVarInt(
                    inventorySource.getFlag().ordinal());
        }
    ...
    public InventoryAction createInventoryAction(Player player) {
        ...
        switch (type) {
            ...
            case CREATIVE -> {
                int createType;
                switch (this.inventorySlot) {
                    case InventoryTransactionPacket.ACTION_MAGIC_SLOT_CREATIVE_DELETE_ITEM -> createType =
                            CreativeInventoryAction.TYPE_DELETE_ITEM;
                    case InventoryTransactionPacket.ACTION_MAGIC_SLOT_CREATIVE_CREATE_ITEM -> createType =
                            CreativeInventoryAction.TYPE_CREATE_ITEM;
                    default -> {
            ...
            case UNTRACKED_INTERACTION_UI, NON_IMPLEMENTED_TODO -> {
                ...
                switch (id) {
                    case SOURCE_TYPE_CRAFTING_ADD_INGREDIENT, SOURCE_TYPE_CRAFTING_REMOVE_INGREDIENT -> {
                        return new SlotChangeAction(
                                player.getCraftingGrid(), this.inventorySlot, this.oldItem, this.newItem);
                    }
                ...
                if (id >= SOURCE_TYPE_ANVIL_OUTPUT && id <= SOURCE_TYPE_ANVIL_INPUT) {
                    ...
                    if (inv instanceof AnvilInventory anvil) {
                        return switch (id) {
                            case SOURCE_TYPE_ANVIL_INPUT,
                                    SOURCE_TYPE_ANVIL_MATERIAL,
                                    SOURCE_TYPE_ANVIL_RESULT -> new RepairItemAction(this.oldItem, this.newItem, id);
                            default -> new SlotChangeAction(anvil, this.inventorySlot, this.oldItem, this.newItem);
                        };
                    } else if ((inv = player.getWindowById(Player.GRINDSTONE_WINDOW_ID))
                            instanceof GrindstoneInventory) {
                        return switch (id) {
                            case SOURCE_TYPE_ANVIL_INPUT,
                                    SOURCE_TYPE_ANVIL_MATERIAL,
                                    SOURCE_TYPE_ANVIL_RESULT -> new GrindstoneItemAction(
                                    this.oldItem,
                                    this.newItem,
                                    id,
                                    id != SOURCE_TYPE_ANVIL_RESULT
                                            ? 0
                                            : ((GrindstoneInventory) inv).getResultExperience());
                ...
                // -17 -15
                    ...
                    return switch (id) {
                        case SOURCE_TYPE_ENCHANT_INPUT -> new EnchantingAction(
                                this.oldItem, this.newItem, SOURCE_TYPE_ENCHANT_INPUT);
                        case SOURCE_TYPE_ENCHANT_MATERIAL -> new EnchantingAction(
                                this.newItem, this.oldItem, SOURCE_TYPE_ENCHANT_MATERIAL); // Mojang ish backwards?
                        case SOURCE_TYPE_ENCHANT_OUTPUT -> new EnchantingAction(
                                this.oldItem, this.newItem, SOURCE_TYPE_ENCHANT_OUTPUT);
