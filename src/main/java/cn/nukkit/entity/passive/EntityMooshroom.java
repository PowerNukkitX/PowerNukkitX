...
public class EntityMooshroom extends EntityAnimal implements EntityWalkable {
        ...
        return new BehaviorGroup(
                ...
                Set.of(
                        new Behavior(
                                ...
                                1),
                        new Behavior(
                                new EntityBreedingExecutor<>(EntityMooshroom.class, 16, 100, 0.5f),
                                entity -> entity.getMemoryStorage().get(CoreMemoryTypes.IS_IN_LOVE),
                                3,
                                1),
                        new Behavior(
                                new MoveToTargetExecutor(CoreMemoryTypes.NEAREST_FEEDING_PLAYER, 0.25f, true),
    ...
    public boolean onInteract(Player player, Item item, Vector3 clickedPos) {
        ...
        if (item.getId() == Item.SHEARS && item.useOn(this)) {
            this.close();
            // TODO 不同颜色的牛掉落不同的蘑菇
            this.getLevel().dropItem(this, Item.get(40, 0, 5));
            this.getLevel().addParticleEffect(this.add(0, this.getHeight(), 0), ParticleEffect.LARGE_EXPLOSION_LEVEL);
            ...
            this.getLevel()
                    .getVibrationManager()
                    .callVibrationEvent(new VibrationEvent(this.clone(), this.getPosition(), VibrationType.SHEAR));
            return true;
