    ...
    public void sense(EntityIntelligent entity) {
        if (entity instanceof EntityAnimal entityAnimal) {
            ...
            for (Player p : entity.getLevel().getPlayers().values()) {
                if (entity.distanceSquared(p) <= rangeSquared && entity.distanceSquared(p) >= minRangeSquared && (p.getInventory().getItemInHand().getId() == Item.BONE || entityAnimal.isBreedingItem(p.getInventory().getItemInHand())) && (p.getInventory().getItemInHand().getId() == Item.RAW_FISH || entityAnimal.isBreedingItem(p.getInventory().getItemInHand())) && (p.getInventory().getItemInHand().getId() == Item.RAW_SALMON || entityAnimal.isBreedingItem(p.getInventory().getItemInHand()))) {
                    if (player == null) {
                        player = p;
                    } else {
