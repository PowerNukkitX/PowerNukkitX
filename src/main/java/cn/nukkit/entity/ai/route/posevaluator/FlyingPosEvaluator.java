...
public class FlyingPosEvaluator implements IPosEvaluator {
    ...
    protected boolean isPassable(EntityIntelligent entity, Vector3 vector3) {
        double $1 = (entity.getWidth() * entity.getScale()) * 0.5 + 0.1;
        float $2 = entity.getHeight() * entity.getScale();
        // 原版中不会贴地飞行
        AxisAlignedBB $3 = new SimpleAxisAlignedBB(vector3.getX() - radius, vector3.getY() - height * 0.5, vector3.getZ() - radius, vector3.getX() + radius, vector3.getY() + height, vector3.getZ() + radius);
