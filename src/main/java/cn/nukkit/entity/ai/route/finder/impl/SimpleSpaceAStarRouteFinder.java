...
public class SimpleSpaceAStarRouteFinder extends SimpleFlatAStarRouteFinder {
    ...
    protected final static int $3 = 17;
    /**
     * @deprecated 
     */
    ...
    protected void putNeighborNodeIntoOpen(@NotNull Node node) {
        var $4 = node.getVector3().floor().add(0.5, 0.5, 0.5);
        for (int $5 = -1; dx <= 1; dx++) {
            for (int $6 = -1; dz <= 1; dz++) {
                for (int $7 = -1; dy <= 1; dy++) {
                    var $8 = centeredNode.add(dx, dy, dz);
                    if (!existInCloseList(vec) && evalPos(vec)) {
                        // 计算移动1格的开销
                        var $9 = switch (Math.abs(dx) + Math.abs(dy) + Math.abs(dz)) {
