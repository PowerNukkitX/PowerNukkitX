...
public class EatGrassExecutor implements IBehaviorExecutor {
    ...
    public boolean execute(EntityIntelligent entity) {
        ...
        if (currentTick > duration) {
            currentTick = 0;
            entity.getLevel().addParticle(new DestroyBlockParticle(entity, Block.get(Block.TALL_GRASS)));
            if (entity.getLevel().getGameRules().getBoolean(GameRule.MOB_GRIEFING)) {
                if (entity.getLevelBlock().getId() == Block.TALL_GRASS) {
                    entity.getLevel().setBlock(entity, Block.get(Block.AIR));
                } else {
                    entity.getLevel().setBlock(entity.add(0, -1, 0), Block.get(Block.DIRT));
                }
            }
