...
public class EntityWarden extends EntityMob implements EntityWalkable, VibrationListener {
        ...
        return new BehaviorGroup(
                ...
                Set.of(
                        new Behavior(
                                (entity) -> {
                                    ...
                                    else this.setAmbientSoundEvent(Sound.MOB_WARDEN_IDLE);
                                    return false;
                                },
                                (entity) -> true,
                                1,
                                ...
                                (entity) -> {
                                    ...
                                    while (iterator.hasNext()) {
                                        ...
                                        if (!entity.level.getName().equals(this.level.getName())
                                            ...
                                            iterator.remove();
                                            var attackTarget =
                                                    this.getMemoryStorage().get(CoreMemoryTypes.ATTACK_TARGET);
                        ...
                        new Behavior(
                                (entity) -> {
                                    // 计算心跳间隔
                                    this.setDataProperty(new IntEntityData(
                                            Entity.DATA_HEARTBEAT_INTERVAL_TICKS, this.calHeartBeatDelay()));
                                    return false;
                                },
                                (entity) -> true,
                                1,
                ...
                Set.of(
                        new Behavior(
                                ...
                                all(
                                        (entity) -> entity.getMemoryStorage()
                                                .compareDataTo(CoreMemoryTypes.IS_ATTACK_TARGET_CHANGED, true),
                        ...
                        new Behavior(
                                ...
                                1),
                        new Behavior(
                                new WardenSniffExecutor((int) (4.2 * 20), 35),
                                new RandomTimeRangeEvaluator(5 * 20, 10 * 20),
                                2),
                        new Behavior(
                                new FlatRandomRoamExecutor(0.1f, 12, 100, true, -1, true, 10), (entity -> true), 1)),
    ...
    protected void initEntity() {
        ...
        this.setDataProperty(
                new IntEntityData(Entity.DATA_HEARTBEAT_SOUND_EVENT, LevelSoundEventPacket.SOUND_HEARTBEAT));
        // 空闲声音
    ...
    public boolean onVibrationOccur(VibrationEvent event) {
        if (Server.getInstance().getTick() - this.lastDetectTime >= 40
                && !waitForVibration
                && !(event.initiator() instanceof EntityWarden)) {
    ...
    public boolean attack(EntityDamageEvent source) {
        ...
        if (cause == EntityDamageEvent.DamageCause.LAVA
                ...
                || cause == EntityDamageEvent.DamageCause.DROWNING) return false;
        if (source instanceof EntityDamageByEntityEvent damageByEntity
                && isValidAngerEntity(damageByEntity.getDamager())) {
    ...
    public void addEntityAngerValue(Entity entity, int addition) {
        ...
        if (added == 0) angerValueMap.remove(entity);
            ...
            boolean changed = attackTarget == null || (entity instanceof Player && !(attackTarget instanceof Player));
            if (changed) {
    ...
    public int calHeartBeatDelay() {
        ...
        var anger =
                this.getMemoryStorage().get(CoreMemoryTypes.WARDEN_ANGER_VALUE).getOrDefault(target, 0);
