 ...
 */
@Slf4j
public record CustomItemDefinition(String identifier, NbtMap nbt) implements BlockID {
    ...
    public static class SimpleBuilder {
        ...
        private NbtMap ensureComponents() {
            if (!nbt.containsKey("components")) {
                nbt = nbt.toBuilder().putCompound("components", NbtMap.EMPTY).build();
            }
            return nbt.getCompound("components");
        }

        protected NbtMap ensureItemProperties() {
            NbtMap components = ensureComponents();
            NbtMap itemProps = components.getCompound("item_properties");
            if (!components.containsKey("item_properties")) {
                itemProps = NbtMap.builder()
        ...
        private NbtMap ensureRepairable() {
            NbtMap components = ensureComponents();
            if (!components.containsKey("minecraft:repairable")) {
                components = components.toBuilder().putCompound("minecraft:repairable", NbtMap.EMPTY).build();
            }
            return components.getCompound("minecraft:repairable");
        }

        private List<NbtMap> ensureRepairItemsList() {
            NbtMap repairable = ensureRepairable();
            List<NbtMap> list = repairable.getList("repair_items", NbtType.COMPOUND);
            if (!repairable.containsKey("repair_items")) {
                repairable = repairable.toBuilder().putList("repair_items", NbtType.COMPOUND, list).build();
            }
            return repairable.getList("repair_items", NbtType.COMPOUND);
        }

        private NbtMap ensureDigger() {
            NbtMap components = ensureComponents();
            if (!components.containsKey("minecraft:digger")) {
                components = components.toBuilder().putCompound("minecraft:digger", NbtMap.EMPTY).build();
            }
            return components.getCompound("minecraft:digger");
        }

        private List<NbtMap> ensureDestroySpeeds() {
            NbtMap digger = ensureDigger();
            List<NbtMap> list = digger.getList("destroy_speeds", NbtType.COMPOUND);
            if (!digger.containsKey("destroy_speeds")) {
                digger = digger.toBuilder().putList("destroy_speeds", NbtType.COMPOUND, list).build();
            }
            return digger.getList("destroy_speeds", NbtType.COMPOUND);
        }
        ...
        private boolean hasTagApplied(String wanted) {
            ...
            if (components.containsKey("item_tags")) {
                var list = components.getList("item_tags", NbtType.STRING);
                for (String st : list) if (wanted.equals(st)) return true;
            }

            if (components.containsKey("minecraft:tags")) {
                NbtMap mcTags = components.getCompound("minecraft:tags");
                if (mcTags.containsKey("tags")) {
                    var list = mcTags.getList("tags", NbtType.STRING);
                    for (String st : list) if (wanted.equals(st)) return true;
                }
            }
            return false;
        ...
        private void addTagIfAbsent(String tag) {
            ...
            List<String> itemTags = components.getList("item_tags", NbtType.STRING);
            itemTags.add(tag);
            components = components.toBuilder().putList("item_tags", NbtType.STRING, itemTags).build();

            NbtMap mcTags = components.getCompound("minecraft:tags");
            List<String> tagsList = mcTags.getList("tags", NbtType.STRING);
            tagsList.add(tag);
        ...
        public SimpleBuilder creativeCategory(CreativeCategory creativeCategory) {
            this.nbt = this.nbt.toBuilder()
                    .putCompound("components", this.ensureComponents().toBuilder()
                            .putCompound("item_properties", this.ensureItemProperties().toBuilder()
        ...
        /**
         * The Creative Group that that includes the specified item. Custom groups can be defined by CreativeCustomGroups. <p>
         * Examples:
         * <pre>
         * // Specificy the item to be part of axes items
         * creativeGroup("minecraft:itemGroup.name.axe");
         * // Specificy the item to be part of a custom group (the group must be registered with CreativeCustomGroups)
        ...
        public SimpleBuilder isHiddenInCommands(boolean hidden) {
            this.nbt = this.nbt.toBuilder()
                    .putCompound("components", this.ensureComponents().toBuilder()
                            .putCompound("item_properties", this.ensureItemProperties().toBuilder()
                                    .putByte("is_hidden_in_commands", hidden ? (byte) 1 : (byte) 0)
                                    .build()
                            ).build()
                    ).build();
            return this;
        ...
        public SimpleBuilder blockPlacer(String blockId, String... useOn) {
            List<NbtMap> useOnList = new ObjectArrayList<>();
            if (useOn != null && useOn.length > 0) {
                for (String s : useOn) {
                    useOnList.add(NbtMap.builder()
                            .putString("name", s)
            ...
            NbtMap blockPlacer = NbtMap.builder()
                    .putString("block", blockId)
                    .putBoolean("canUseBlockAsIcon", true)
                    .putList("use_on", NbtType.COMPOUND, useOnList)
                    .build();

            NbtMap updated = ensureComponents().toBuilder()
                    .putCompound("minecraft:block_placer", blockPlacer)
                    .putCompound("minecraft:publisher_on_use_on",
                            NbtMap.builder().putBoolean("autoSucceedOnClient", false).build()
                    ).build();

            this.nbt = this.nbt.toBuilder().putCompound("components", updated).build();
            return this;
        }
        ...
        public SimpleBuilder glint(boolean glint) {
            this.nbt = this.nbt.toBuilder()
                    .putCompound("components", this.ensureComponents().toBuilder()
                            .putCompound("minecraft:glint", NbtMap.builder()
                                    .putByte("value", glint ? (byte) 1 : 0)
                                    .build()
                            )
                            .putCompound("item_properties", this.ensureItemProperties().toBuilder()
                                    .putBoolean("foil", glint)
                                    .build()
                            ).build()
                    ).build();
            return this;
        ...
        public SimpleBuilder allowOffHand(boolean allowOffHand) {
            this.nbt = this.nbt.toBuilder()
                    .putCompound("components", this.ensureComponents().toBuilder()
                            .putCompound("minecraft:allow_off_hand", NbtMap.builder()
                                    .putByte("value", allowOffHand ? (byte) 1 : 0)
                                    .build()
                            )
                            .putCompound("item_properties", this.ensureItemProperties().toBuilder()
        ...
        public SimpleBuilder tag(String... tags) {
            ...
            final List<String> itemTags = new ObjectArrayList<>(unique);
            this.nbt = this.nbt.toBuilder()
                    .putCompound("components", this.ensureComponents().toBuilder()
                            .putList("item_tags", NbtType.STRING, itemTags)
                            .putCompound("minecraft:tags", NbtMap.builder()
                                    .putList("tags", NbtType.STRING, itemTags)
                                    .build()
                            ).build()
                    ).build();
            return this;
        ...
        public SimpleBuilder enchantable(String slot, int value) {
            ...
            this.nbt = this.nbt.toBuilder()
                    .putCompound("components", this.ensureComponents().toBuilder()
                            .putCompound("minecraft:enchantable", NbtMap.builder()
                                    .putString("slot", slot)
                                    .putByte("value", (byte) value)
                                    .build()
                            ).putCompound("item_properties", this.ensureItemProperties().toBuilder()
                                    .putString("enchantable_slot", slot)
                                    .putInt("enchantable_value", value)
                                    .build()
                            ).build()
                    ).build();
            return this;
        ...
        public SimpleBuilder digger(DiggerEntry... diggerEntries) {
            ...
            if (this.diggerUseEfficiency != null) {
                this.nbt = this.nbt.toBuilder()
                        .putCompound("components", this.ensureComponents().toBuilder()
                                .putCompound("minecraft:digger", this.ensureDigger().toBuilder()
                                        .putBoolean("use_efficiency", this.diggerUseEfficiency)
                                        .build()
                                ).build()
                        ).build();
            }
        ...
        public SimpleBuilder useEfficiency(boolean useEfficiency) {
            ...
            this.nbt = this.nbt.toBuilder()
                    .putCompound("components", this.ensureComponents().toBuilder()
                            .putCompound("minecraft:digger", this.ensureDigger().toBuilder()
                                    .putBoolean("use_efficiency", useEfficiency)
                                    .build()
                            ).build()
                    ).build();
            return this;
        ...
        public SimpleBuilder shooter(float maxDrawDuration,
            ...
            if (ammunition != null && ammunition.length > 0) {
                final List<NbtMap> ammoList = new ObjectArrayList<>();
                for (ShooterAmmo a : ammunition) {
                    if (a != null) ammoList.add(a.toNbt());
                }
                if (!ammoList.isEmpty()) shooter.putList("ammunition", NbtType.COMPOUND, ammoList);
            }

            this.nbt = this.nbt.toBuilder()
                    .putCompound("components", this.ensureComponents().toBuilder()
                            .putCompound("minecraft:shooter", shooter.build())
                            .build()
                    ).build();
            return this;
        }
        ...
        public SimpleBuilder throwable(boolean doSwingAnimation,
            ...
            final NbtMap throwable = NbtMap.builder()
            ...
            this.nbt = this.nbt.toBuilder()
                    .putCompound("components", this.ensureComponents().toBuilder()
                            .putCompound("minecraft:throwable", throwable)
                            .build()
                    ).build();
            return this;
        }
        ...
        public SimpleBuilder fuel(float duration) {
            ...
            this.nbt = this.nbt.toBuilder()
                    .putCompound("components", this.ensureComponents().toBuilder()
                            .putCompound("minecraft:fuel", NbtMap.builder()
         ...
         */
        public CustomItemDefinition customBuild(Consumer<NbtMap> nbtConsumer) {
            var def = this.build();
        ...
        protected CustomItemDefinition buildDefinition() {
            NbtMap itemProps = ensureItemProperties();
            NbtMap components = ensureComponents();
        ...
        private void writeIcon(NbtMap itemProps) {
            if (texture != null && !texture.isBlank()) {
                final NbtMap icon = NbtMap.builder()
                ...
                this.nbt = this.nbt.toBuilder()
                        .putCompound("components", this.ensureComponents().toBuilder()
                                .putCompound("item_properties", itemProps.toBuilder()
                                        .putCompound("minecraft:icon", icon)
                                        .build()
                                ).build()
                        ).build();
            }
        ...
        private void writeDisplayName(NbtMap components) {
            if (name != null) {
                final NbtMap displayName = NbtMap.builder()
                        .putString("value", this.name)
                        .build();
                this.nbt = this.nbt.toBuilder()
                        .putCompound("components", components.toBuilder()
                                .putCompound("minecraft:display_name", displayName)
                                .build()
                        ).build();
            }
        }

        private void writeStackSize(NbtMap itemProps, NbtMap components) {
        ...
        private void writeBaseItemProps(NbtMap itemProps) {
            this.nbt = this.nbt.toBuilder()
                    .putCompound("components", this.ensureComponents().toBuilder()
                            .putCompound("item_properties", itemProps.toBuilder()
                                    .putFloat("mining_speed", this.miningSpeed != null ? this.miningSpeed : 1.0f)
                                    .putBoolean("should_despawn", shouldDespawn)
                                    .putBoolean("stacked_by_data", this.stackedByData != null ? this.stackedByData : false)
                                    .build()
                            ).build()
                    ).build();
        }

        private void writeUseAnimationAndModifiers(NbtMap itemProps, NbtMap components) {
            final NbtMapBuilder componentsBuilder = components.toBuilder();

            if (this.useAnimationType != null) {
                componentsBuilder.putCompound("minecraft:use_animation",
                        NbtMap.builder().putString("value", this.useAnimationType).build());
            }

            if (this.useModifierMovement != null && this.useModifierDuration != null) {
                componentsBuilder.putCompound("minecraft:use_modifiers",
                        NbtMap.builder()
            ...
            final NbtMapBuilder itemPropsBuilder = itemProps.toBuilder();

            final int animationId = resolveAnimationId(this.useAnimationType);
            itemPropsBuilder.putInt("use_animation", animationId);

            final int useDurationTicks = toTicks(this.useModifierDuration);
            itemPropsBuilder.putInt("use_duration", useDurationTicks);

            componentsBuilder.putCompound("item_properties", itemPropsBuilder.build());

            this.nbt = this.nbt.toBuilder().putCompound("components", componentsBuilder.build()).build();
        }

        private void writeCooldown(NbtMap components) {
            if (cooldownCategory != null && cooldownDuration != null) {
                this.nbt = this.nbt.toBuilder()
                        .putCompound("components", components.toBuilder()
                                .putCompound("minecraft:cooldown", NbtMap.builder()
        ...
        private void writeDurability(NbtMap components) {
            if (this.maxDurability == null) return;

            final NbtMapBuilder durability = NbtMap.builder().putInt("max_durability", this.maxDurability);
            ...
            this.nbt = this.nbt.toBuilder()
                    .putCompound("components", components.toBuilder()
                            .putCompound("minecraft:durability", durability.build())
                            .build()
                    ).build();
        }

        private void writeDamage(NbtMap itemProps, NbtMap components) {
            if (this.damage == null) return;

            this.nbt = this.nbt.toBuilder()
                    .putCompound("components", components.toBuilder()
                            .putCompound("minecraft:damage", NbtMap.builder()
                                    .putByte("value", Integer.valueOf(this.damage.intValue() & 0xFF).byteValue())
                                    .build()
                            ).putCompound("item_properties", itemProps.toBuilder()
                                    .putInt("damage", this.damage)
                                    .build()
                            ).build()
                    ).build();
        }

        private void writeFood(NbtMap components, NbtMap itemProps) {
            boolean hasFood =
            ...
            final NbtMapBuilder itemPropsBuilder = itemProps.toBuilder();
            final NbtMapBuilder componentsBuilder = components.toBuilder();
            final NbtMapBuilder food = NbtMap.builder();
            ...
            componentsBuilder.putCompound("minecraft:food", food.build());

            // Default use_modifiers for food if none provided
            if (this.useModifierMovement == null && this.useModifierDuration == null) {
                itemPropsBuilder.putInt("use_duration", 0);
                componentsBuilder.putCompound("minecraft:use_modifiers",
                        NbtMap.builder()
        ...
        private void writeWearable(NbtMap components) {
        ...
        private void applyAutoToolTag(NbtMap itemProps) {
        ...
        protected SimpleBuilder addRepairs(@NotNull List<String> repairItemNames, String molang) {
            ...
            NbtMapBuilder componentsBuilder = nbt.getCompound("components").toBuilder();
            NbtMap repairable = componentsBuilder.build().getCompound("minecraft:repairable");

            if (!componentsBuilder.containsKey("minecraft:repairable")) {
                repairable = NbtMap.EMPTY;
                componentsBuilder.putCompound("minecraft:repairable", repairable);
            }

            List<NbtMap> repairItems = repairable.getList("repair_items", NbtType.COMPOUND);
            if (repairItems == null) repairItems = new ObjectArrayList<>();

            List<NbtMap> items = new ObjectArrayList<>();
            for (String name : repairItemNames) {
                items.add(NbtMap.builder().putString("name", name).build());
            }
            ...
            repairable = repairable.toBuilder().putList("repair_items", NbtType.COMPOUND, repairItems).build();
            this.nbt = this.nbt.toBuilder()
                    .putCompound("components",
                            componentsBuilder.putCompound("minecraft:repairable", repairable).build()
                    ).build();
            return this;
        }
        ...
        public SimpleBuilder renderOffsets(@NotNull RenderOffsets renderOffsets) {
            ...
            if (!nbt.containsKey("components")) {
                ...
                this.nbt = nbt.toBuilder().putCompound("components", components).build();
            }
            this.nbt = this.nbt.toBuilder()
                    .putCompound("components", components.toBuilder()
                            .putCompound("minecraft:render_offsets", renderOffsets.getNbt())
                            .build())
                    .build();
            return this;
        }
    ...
    public BlockPlacerData getBlockPlacerData() {
        NbtMap components = nbt.getCompound("components");
        if (!components.containsKey("minecraft:block_placer")) {
            return null;
        }

        NbtMap placer = components.getCompound("minecraft:block_placer");
        String blockId = placer.getString("block");
        List<String> useOnList = new ArrayList<>();

        List<NbtMap> useOnTag = placer.getList("use_on", NbtType.COMPOUND);
        if (useOnTag != null && useOnTag.size() > 0) {
            for (int i = 0; i < useOnTag.size(); i++) {
                NbtMap entry = useOnTag.get(i);
    ...
    public @Nullable String getDisplayName() {
        if (!this.nbt.getCompound("components").containsKey("minecraft:display_name")) return null;
    ...
    public boolean hasComponent(String key) {
        NbtMap comps = getComponents();
        return comps != null && comps.containsKey(key);
    }

    public NbtMap getComponent(String key) {
        NbtMap comps = getComponents();
        return (comps != null && comps.containsKey(key)) ? comps.getCompound(key) : null;
    }

    public NbtMap getItemProperties() {
        NbtMap comps = getComponents();
    ...
    public @NotNull ItemArmorType getWearableType() {
        NbtMap wearable = getComponent("minecraft:wearable");
    ...
    public boolean isHelmet() {
        return getWearableType() == ItemArmorType.HEAD;
    }

    public boolean isChestplate() {
        return getWearableType() == ItemArmorType.CHEST;
    }

    public boolean isLeggings() {
        return getWearableType() == ItemArmorType.LEGS;
    }

    public boolean isBoots() {
        return getWearableType() == ItemArmorType.FEET;
    }

    public @Nullable ItemEnchantSlot getEnchantSlot() {
        NbtMap itemProps = getComponent("item_properties");
    ...
    public boolean isSword() {
        return getEnchantSlot() == ItemEnchantSlot.SWORD;
    }

    public boolean isMace() {
        return getEnchantSlot() == ItemEnchantSlot.MACE;
    }

    public boolean isShield() {
        return getEnchantSlot() == ItemEnchantSlot.SHIELD;
    }

    public boolean isPickaxe() {
        return getEnchantSlot() == ItemEnchantSlot.PICKAXE;
    }

    public boolean isShovel() {
        return getEnchantSlot() == ItemEnchantSlot.SHOVEL;
    }

    public boolean isAxe() {
        return getEnchantSlot() == ItemEnchantSlot.AXE;
    }

    public boolean isHoe() {
        return getEnchantSlot() == ItemEnchantSlot.HOE;
    }

    public boolean isShears() {
        return getEnchantSlot() == ItemEnchantSlot.SHEARS;
    }

    public boolean isBow() {
        return getEnchantSlot() == ItemEnchantSlot.BOW;
    }

    public boolean isCrossbow() {
        return getEnchantSlot() == ItemEnchantSlot.CROSSBOW;
    }

    public boolean isTrident() {
        return getEnchantSlot() == ItemEnchantSlot.TRIDENT;
    }

    public boolean isSpear() {
        return getEnchantSlot() == ItemEnchantSlot.SPEAR;
    }
