...
public class JeMapping {
    private static final Map<JeBlockState, Integer> JE_BLOCK_STATE_TO_BE_BLOCK_STATE_HASH_MAP =
            new Object2IntOpenHashMap<>();
    private static final Map<Integer, JeBlockState> BE_BLOCK_STATE_HASH_TO_JE_BLOCK_STATE_MAP =
    ...
    @Nullable public static String getJeBiomeNameByBeBiomeId(int beBiomeId) {
        return BE_TO_JE_BIOME_MAP.get(beBiomeId);
    }

    @Nullable public static Integer getBeBiomeIdByJeBiomeName(String jeBiomeName) {
        return JE_TO_BE_BIOME_MAP.get(jeBiomeName);
    }

    @Nullable public static JeBlockState getJeBlockStateByBeBlockStateHash(int beBlockStateHash) {
        return BE_BLOCK_STATE_HASH_TO_JE_BLOCK_STATE_MAP.get(beBlockStateHash);
    }

    @Nullable public static Integer getBeBlockStateHashByJeBlockState(JeBlockState jeBlockState) {
    ...
    private static void init() {
        ...
        try {
            jeBlocksMappingConfig.load(
                    JeMapping.class.getModule().getResourceAsStream("jeMappings/jeBlocksMapping.json"));
        } catch (IOException e) {
        ...
        jeBlocksMappingConfig.getAll().forEach((k, v) -> {
            ...
            var hash = computeBedrockBlockStateHash(
                    (String) valueMap.get("bedrock_identifier"), (Map<String, Object>) valueMap.get("bedrock_states"));
            BE_BLOCK_STATE_HASH_TO_JE_BLOCK_STATE_MAP.put(hash, new JeBlockState(k));
            JE_BLOCK_STATE_TO_BE_BLOCK_STATE_HASH_MAP.put(new JeBlockState(k), hash);
        });

        // Biome
        final var jeBiomesMappingConfig = new Config(Config.JSON);
        try {
            jeBiomesMappingConfig.load(
                    PNXWorldHandle.class.getModule().getResourceAsStream("jeMappings/jeBiomesMapping.json"));
        } catch (IOException e) {
            throw new RuntimeException(e);
        }
