...
public class BlockStone extends BlockSolidMeta {
    @PowerNukkitOnly
    @Since("1.4.0.0-PN")
    public static final BlockProperty<StoneType> STONE_TYPE =
            new ArrayBlockProperty<>("stone_type", true, StoneType.class);
    ...
    public Item[] getDrops(Item item) {
        if (item.isPickaxe() && item.getTier() >= getToolTier()) {
            return new Item[] {StoneType.STONE.equals(getStoneType()) ? Item.getBlock(BlockID.COBBLESTONE) : toItem()};
        } else {
