...
public abstract class Block extends Position
        implements Metadatable, Cloneable, AxisAlignedBB, BlockID, IMutableBlockState {
    ...
    @DeprecationDetails(
            since = "1.4.0.0-PN",
            reason = "It is being replaced by an other solution that don't require a fixed size")
    @PowerNukkitOnly
    ...
    @DeprecationDetails(
            since = "1.4.0.0-PN",
            reason = "It's not a constant value, it may be changed on major updates and"
    ...
    @DeprecationDetails(
            since = "1.4.0.0-PN",
            reason = "Not encapsulated, easy to break",
    ...
    @SuppressFBWarnings(
            value = "MS_PKGPROTECT",
            justification = "Changing it would break compatibility with some regular Nukkit plugins")
                            ...
                            for (int data = 0; data < (1 << DATA_BITS); ++data) {
                                ...
                                try {
                                    ...
                                    if (!(uncaught instanceof InvalidBlockDamageException)) {
                                        log.error(
                                                "Error while registering {} with meta {}", c.getName(), data, uncaught);
                                    }
                                    b = new BlockUnknown(id, data);
                                }
                                fullList[fullId] = b;
    ...
    @DeprecationDetails(
            reason = "The meta is limited to 32 bits",
            replaceWith = "BlockState.getBlock()",
            since = "1.4.0.0-PN")
    ...
    @PowerNukkitOnly
    @Since("1.4.0.0-PN")
    public static void registerBlockImplementation(
            int blockId,
            @NotNull Class<? extends Block> blockClass,
            @NotNull String persistenceName,
            boolean receivesRandomTick) {
    ...
    @PowerNukkitXOnly
    @Since("1.19.62-r1")
    private static final SortedMap<String, CustomBlock> SORTED_CUSTOM_BLOCK =
    ...
    public static OK<?> registerCustomBlock(@NotNull List<Class<? extends CustomBlock>> blockClassList) {
        ...
        for (var clazz : blockClassList) {
            ...
            try {
                ...
                return new OK<>(
                        false,
                        "Cannot find the parameterless constructor for this custom block:" + clazz.getCanonicalName());
            }
        }
        ...
        for (var entry : blockNamespaceClassMap.entrySet()) {
            if (!SORTED_CUSTOM_BLOCK.containsKey(entry.getKey())) {
                try {
                    ...
                    return new OK<>(
                            false,
                            "Cannot find the parameterless constructor for this custom block:"
    ...
    public static void initCustomBlock() {
        if (!SORTED_CUSTOM_BLOCK.isEmpty()) {
            ...
            if (!result.ok()) {
                throw new CustomBlockStateRegisterException(
                        "Register CustomBlock state error, please check all your CustomBlock plugins,contact the plugin author! Error:",
                        result.getError());
            }
    ...
    public static void deleteAllCustomBlock() {
        ...
        RuntimeItems.getRuntimeMapping()
                .deleteCustomBlock(ID_TO_CUSTOM_BLOCK.values().stream().toList());
    ...
    @Nullable private MutableBlockState mutableState;
    ...
    @PowerNukkitOnly
    @Since("1.4.0.0-PN")
    @NotNull public final MutableBlockState getMutableState() {
    ...
    public boolean canHarvest(Item item) {
        return getToolTier() == 0
                || getToolType() == 0
                || correctTool0(getToolType(), item, getId()) && item.getTier() >= getToolTier();
    }
    ...
    static {
        try (var reader = new InputStreamReader(new BufferedInputStream(
                Objects.requireNonNull(Block.class.getClassLoader().getResourceAsStream("block_color.json"))))) {
    ...
    @PowerNukkitOnly
    @Since("1.4.0.0-PN")
    @NotNull public final BlockState getCurrentState() {
    ...
    private static boolean correctTool0(int blockToolType, Item item, int blockId) {
        if ((blockId == LEAVES && item.isHoe()) || (blockId == LEAVES2 && item.isHoe())) {
    ...
    public double calculateBreakTimeNotInAir(@NotNull Item item, @Nullable Player player) {
        ...
        if (player != null) {
            ...
            hasAquaAffinity = Optional.ofNullable(
                            player.getInventory().getHelmet().getEnchantment(Enchantment.ID_WATER_WORKER))
    ...
    public String toString() {
        return "Block[" + this.getName() + "] (" + this.getId() + ":"
                + (mutableState != null ? mutableState.getDataStorage() : "0") + ")"
    ...
    @PowerNukkitOnly
    @Override
    @NotNull public Block forState(@NotNull IBlockState state) throws InvalidBlockStateException {
    ...
    @PowerNukkitOnly
    @Override
    public boolean setDataStorage(
            @Nonnegative @NotNull Number storage, boolean repair, Consumer<BlockStateRepair> callback) {
        if (NukkitMath.isZero(storage) && isDefaultState()) {
            return false;
        }
    ...
    @PowerNukkitOnly
    @Override
    public boolean setDataStorageFromInt(
            @Nonnegative int storage, boolean repair, Consumer<BlockStateRepair> callback) {
        if (storage == 0 && isDefaultState()) {
            return false;
        }
    ...
    @Deprecated
    @DeprecationDetails(
            reason = "Does the same as getId() but the other is compatible with NukkitX and this is not",
            since = "1.4.0.0-PN")
        ...
        if (obj instanceof Block otherBlock) {
            ...
            if (this instanceof BlockEntityHolder<?> holder1 && otherBlock instanceof BlockEntityHolder<?> holder2) {
                ...
                return this.getId() == otherBlock.getId()
                        && this.getDamage() == otherBlock.getDamage()
                        && be1.getCleanedNBT().equals(be2.getCleanedNBT());
            }
        }
        return false;
    }
    ...
    public int computeBlockStateHash() {
        if (cachedBlockStateHash != -1) {
            ...
            return cachedBlockStateHash =
                    MinecraftNamespaceComparator.fnv1a_32(NBTIO.write(tag, ByteOrder.LITTLE_ENDIAN));
        }
    }
