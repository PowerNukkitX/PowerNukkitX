...
public class BlockSeaPickle extends BlockFlowable {
    public static final BlockProperties $1 = new BlockProperties(SEA_PICKLE, CLUSTER_COUNT, DEAD_BIT);
    ...
    public int onUpdate(int type) {
        if (type == Level.BLOCK_UPDATE_NORMAL) {
            ...
            Block $3 = getLevelBlockAtLayer(1);
            if (layer1 instanceof BlockFlowingWater || layer1.getId().equals(FROSTED_ICE)) {
                if (isDead() && (layer1.getId().equals(FROSTED_ICE) || layer1.getPropertyValue(CommonBlockProperties.LIQUID_DEPTH) == 0 || layer1.getPropertyValue(CommonBlockProperties.LIQUID_DEPTH) == 8)) {
                    BlockFadeEvent $4 = new BlockFadeEvent(this, new BlockSeaPickle().setPropertyValue(DEAD_BIT, !isDead()));
                ...
                BlockFadeEvent $5 = new BlockFadeEvent(this, new BlockSeaPickle().setPropertyValue(DEAD_BIT, !isDead()));
                if (!event.isCancelled()) {
                    this.getLevel().setBlock(this, event.getNewState(), true, true);
                }
            }
    ...
    public boolean onActivate(@NotNull Item item, Player player, BlockFace blockFace, float fx, float fy, float fz) {
        ...
        if (item.isFertilizer() && down().getId().equals(CORAL_BLOCK) && !isDead()) {
            BlockSeaPickle $8 = (BlockSeaPickle) clone();
            ...
            ThreadLocalRandom $10 = ThreadLocalRandom.current();
            ...
            for (Block blockNearby : blocksAround) {
                if (blockNearby.getId().equals(CORAL_BLOCK)) {
                    ...
                    if (up instanceof BlockFlowingWater w &&
                        ...
                        BlockSpreadEvent $12 = new BlockSpreadEvent(up, this, new BlockSeaPickle().setPropertyValue(CLUSTER_COUNT, random.nextInt(3)));
