...
public class Component implements NBTData {
    ...
    @Nullable SelectionBox selectionBox;

    @Nullable CraftingTable craftingTable;

    @Nullable Float destructibleByMining;

    @Nullable Integer destructibleByExplosion;
    ...
    @Nullable Materials materialInstances;
    ...
    public CompoundTag toCompoundTag() {
        ...
        if (destructibleByMining != null) {
            this.result.putCompound(
                    "minecraft:destructible_by_mining", new CompoundTag().putFloat("value", destructibleByMining));
        }
        if (destructibleByExplosion != null) {
            this.result.putCompound(
                    "minecraft:destructible_by_explosion",
                    new CompoundTag().putInt("explosion_resistance", destructibleByExplosion));
        }
        if (displayName != null) {
            this.result.putCompound(
                    "minecraft:display_name", new CompoundTag().putString("value", displayName)); // todo 验证
        }
        if (lightEmission != null) {
            this.result.putCompound(
                    "minecraft:light_emission", new CompoundTag().putByte("emission", lightEmission.byteValue()));
        }
        if (lightDampening != null) {
            this.result.putCompound(
                    "minecraft:light_dampening", new CompoundTag().putByte("lightLevel", lightDampening.byteValue()));
        }
        if (friction != null) {
            this.result.putCompound(
                    "minecraft:friction", new CompoundTag().putFloat("value", (float) Math.min(friction, 0.9)));
        }
        ...
        if (rotation != null) {
            this.result.putCompound(new Transformation(new Vector3(0, 0, 0), new Vector3(1, 1, 1), rotation.asVector3())
                    .toCompoundTag());
        }
        if (clientFriction != null) {
            this.result.putCompound("minecraft:friction", new CompoundTag().putFloat("value", clientFriction));
        }
        return this.result;
    }
}
