...
@Log4j2
public abstract class BlockPistonBase extends BlockTransparentMeta
        implements Faceable, RedstoneComponent, BlockEntityHolder<BlockEntityPistonArm> {
    ...
    @PowerNukkitDifference(
            info = "Using new method for checking if powered + update all around redstone torches, "
                    + "even if the piston can't move.",
            since = "1.4.0.0-PN")
    ...
    protected boolean checkState(Boolean isPowered) {
        ...
        if (block instanceof BlockPistonHead b) {
            if (b.getBlockFace() != face) return false;
            isExtended = true;
        } else isExtended = false;
        if (isPowered && !isExtended) {
            if (!this.doMove(true)) return false;
            this.getLevel().addSound(this, Sound.TILE_PISTON_OUT);
            this.getLevel()
                    .getVibrationManager()
                    .callVibrationEvent(new VibrationEvent(this, this.add(0.5, 0.5, 0.5), VibrationType.PISTON_EXTEND));
            return true;
        } else if (!isPowered && isExtended) {
            if (!this.doMove(false)) return false;
            this.getLevel().addSound(this, Sound.TILE_PISTON_IN);
            this.getLevel()
                    .getVibrationManager()
                    .callVibrationEvent(
                            new VibrationEvent(this, this.add(0.5, 0.5, 0.5), VibrationType.PISTON_CONTRACT));
            return true;
        }
        return false;
    }
    ...
    protected boolean doMove(boolean extending) {
        ...
        if (!canMove && extending) return false;
    ...
    public class BlocksCalculator {
        ...
        private final List<Block> toMove = new ArrayList<>() {
            ...
            public int indexOf(Object o) {
                if (o == null) {
                    for (int i = 0; i < size(); i++) if (get(i) == null) return i;
                } else {
        ...
        public boolean canMove() {
            if (!sticky && !extending) return true;
            this.toMove.clear();
            this.toDestroy.clear();
            var block = this.blockToMove;
            if (!canPush(block, this.moveDirection, true, extending)) return false;
            if (block.breaksWhenMoved()) {
                if (extending || block.sticksToPiston()) this.toDestroy.add(this.blockToMove);
                return true;
            }
        ...
        protected boolean addBlockLine(Block origin, Block from, boolean mainBlockLine) {
            ...
            while (block.canSticksBlock()) {
                ...
                if ((!extending || !mainBlockLine)
                        && block.canSticksBlock()
                        && oldBlock.canSticksBlock()
                        && block.getId() != oldBlock.getId()) break;
                if (block.getId() == AIR
                        || !canPush(block, this.moveDirection, false, extending)
                        || block.equals(this.pistonPos)) break;
                if (block.breaksWhenMoved() && block.sticksToPiston()) {
                    this.toDestroy.add(block);
                    break;
                }
                if (count + this.toMove.size() > MOVE_BLOCK_LIMIT) return false;
                count++;
                beStuck.add(block);
            }
            int beStuckCount = beStuck.size();
            if (beStuckCount > 0) this.toMove.addAll(Lists.reverse(beStuck));
            int step = 1;
            while (true) {
                ...
                if (index > -1) {
                    ...
                    for (int i = 0; i <= index + beStuckCount; ++i) {
                        var b = this.toMove.get(i);
                        if ((b.canSticksBlock()) && !this.addBranchingBlocks(b)) return false;
                    }
                    return true;
                }
                if (nextBlock.getId() == AIR || nextBlock.equals(armPos)) return true;
        ...
        protected boolean addBranchingBlocks(Block block) {
            for (BlockFace face : BlockFace.values()) {
                if (face.getAxis() != this.moveDirection.getAxis()
                        && !this.addBlockLine(block.getSide(face), block, false)) return false;
            }
            return true;
        }
