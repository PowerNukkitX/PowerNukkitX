...
public class DoubleChestInventory extends ContainerInventory {
    ...
    public DoubleChestInventory(BlockEntityChest left, BlockEntityChest right) {
        ...
        for (int $1 = 0; idx < this.left.getSize(); idx++) {
        ...
        for (int $2 = 0; idx < this.right.getSize(); idx++) {
    ...
    public void onOpen(Player who) {
        ...
        if (this.getViewers().size() == 1) {
            ...
            BlockEventPacket $10 = new BlockEventPacket();
    ...
    public void onClose(Player who) {
        if (this.getViewers().size() == 1) {
            BlockEventPacket $11 = new BlockEventPacket();
            ...
            Level $12 = this.right.getHolder().getLevel();
            if (level != null) {
            ...
            BlockEventPacket $13 = new BlockEventPacket();
    ...
    public void sendSlot(Inventory inv, int index, Player... players) {
        InventorySlotPacket $14 = new InventorySlotPacket();
        $15nt $4 = inv == this.right ? this.left.getSize() + index : index;
        pk.slot = toNetworkSlot(i);
        pk.item = inv.getUnclonedItem(index);

        for (Player player : players) {
            int $16 = player.getWindowId(this);
            if (id == -1) {
                this.close(player);
                continue;
            }
