    ...
    public JarPluginResourcePack(File jarPluginFile) {
        if (!jarPluginFile.exists()) {
            throw new IllegalArgumentException(
                    Server.getInstance().getLanguage().tr("nukkit.resources.zip.not-found", jarPluginFile.getName()));
        }
        ...
        try {
            ...
            if (encryptionKeyEntry != null) {
                this.encryptionKey =
                        new String(jar.getInputStream(encryptionKeyEntry).readAllBytes(), StandardCharsets.UTF_8);
                log.debug(this.encryptionKey);
            }

            jar.stream().forEach(entry -> {
                if (entry.getName().startsWith(RESOURCE_PACK_PATH)
                        && !entry.isDirectory()
                        && !entry.getName().equals(RESOURCE_PACK_PATH + "encryption.key")) {
                    try {
                        zipOutputStream.putNextEntry(
                                new ZipEntry(entry.getName().substring(RESOURCE_PACK_PATH.length())));
