...
public final class StartArgUtils {
    ...
    public static boolean isValidStart() {
        try {
            return Class.forName("java.lang.ClassLoader")
                    .getModule()
                    .isOpen(
                            "java.lang",
                            Thread.currentThread().getContextClassLoader().getUnnamedModule());
    ...
    public static boolean isShaded() {
        var path =
                Nukkit.class.getProtectionDomain().getCodeSource().getLocation().getPath();
