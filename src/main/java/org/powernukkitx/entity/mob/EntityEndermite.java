...
public class EntityEndermite extends EntityMob implements EntityWalkable, EntityArthropod {
    ...
    public IBehaviorGroup requireBehaviorGroup() {
        return EntityAI.of(this)
                .behavior(new PlaySoundExecutor(Sound.MOB_ENDERMITE_SAY))
                        .when(new RandomSoundEvaluator())
                .behavior(new MeleeAttackExecutor(CoreMemoryTypes.ATTACK_TARGET, 0.3f, 16, true, 30))
                        .when(new EntityCheckEvaluator(CoreMemoryTypes.ATTACK_TARGET))
                .behavior(new MeleeAttackExecutor(CoreMemoryTypes.NEAREST_SHARED_ENTITY, 0.3f, 16, true, 30))
                        .when(new EntityCheckEvaluator(CoreMemoryTypes.NEAREST_SHARED_ENTITY))
                .behavior(new MeleeAttackExecutor(CoreMemoryTypes.NEAREST_PLAYER, 0.3f, 16, false, 30))
