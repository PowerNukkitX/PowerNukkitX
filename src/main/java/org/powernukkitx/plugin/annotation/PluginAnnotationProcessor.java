...
@SupportedAnnotationTypes({
        "org.powernukkitx.plugin.annotation.PluginMeta",
        "org.powernukkitx.plugin.annotation.EventListener",
        "org.powernukkitx.plugin.annotation.ScheduleTask",
        "org.powernukkitx.plugin.annotation.CommandDefinition"
})
public class PluginAnnotationProcessor extends AbstractProcessor {
    ...
    private void collectListener(Element e, List<String> out) {
        ...
        if (!isAssignableTo(type, "org.powernukkitx.event.Listener")) {
            error(e, "@EventListener class must implement org.powernukkitx.event.Listener.");
            return;
        }
    ...
    private void collectTask(Element e, List<ClassTask> classOut, List<MethodTask> methodOut) {
        ...
        if (e.getKind() == ElementKind.CLASS) {
            ...
            if (!isAssignableTo(type, "java.lang.Runnable")) {
                error(e, "@ScheduleTask class must implement Runnable (e.g. extend org.powernukkitx.scheduler.Task).");
                return;
            }
    ...
    private void collectCommand(Element e, List<CommandEntry> out) {
        ...
        if (!isAssignableTo(type, "org.powernukkitx.command.Command")) {
            error(e, "@CommandDefinition class must extend org.powernukkitx.command.Command.");
            return;
        }
    ...
    private boolean validateMain(TypeElement main) {
        ...
        if (!isAssignableTo(main, "org.powernukkitx.plugin.PluginBase")) {
            error(main, "@PluginMeta class must extend org.powernukkitx.plugin.PluginBase.");
            return false;
        }
        return true;
    }
    ...
    private void writeBootstrap(TypeElement main, String pkg, String bootstrapFqn,
        ...
        sb.append("    public static void init(org.powernukkitx.plugin.Plugin plugin) {\n");
        if (!listeners.isEmpty()) {
            sb.append("        org.powernukkitx.plugin.PluginManager __pm = plugin.getServer().getPluginManager();\n");
        }
        if (!classTasks.isEmpty() || !methodTasks.isEmpty()) {
            sb.append("        org.powernukkitx.scheduler.ServerScheduler __sch = plugin.getServer().getScheduler();\n");
        }
        ...
        if (!commands.isEmpty()) {
            sb.append("        org.powernukkitx.command.CommandMap __cmap = plugin.getServer().getCommandMap();\n");
