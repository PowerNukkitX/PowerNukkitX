...
public class ExitPortalPopulator extends Populator {
    ...
    public void apply(ChunkGenerateContext context) {
        ...
        if(chunkX == 1 && chunkZ == 0) {
            int portalY = chunk.getHeightMap(0, 0) - 1;
            BlockManager object = new BlockManager(level);
            ObjectExitPortal exitPortal = new ObjectExitPortal(false);
            exitPortal.generate(object, null, new Vector3(0, portalY, 0));
            object.applySubChunkUpdate();

            var fight = level.getEndDragonFight();
            if (fight != null) {
                fight.onExitPortalGenerated(new BlockVector3(0, portalY, 0));
            }
        }
    }
