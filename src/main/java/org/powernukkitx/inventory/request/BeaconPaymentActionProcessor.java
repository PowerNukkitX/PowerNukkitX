...
public class BeaconPaymentActionProcessor implements ItemStackRequestActionProcessor<BeaconPaymentAction> {
    ...
    public ActionResponse handle(BeaconPaymentAction action, Player player, ItemStackRequestContext context) {
        ...
        // The payment is taken here instead of relying on a DestroyAction later in the request: the
        // client decides which actions it sends, so anything it is trusted to do can also be left out.
        beaconInventory.decreaseCount(0, 1);
        context.markServerConsumed(ContainerEnumName.BEACON_PAYMENT_CONTAINER);
